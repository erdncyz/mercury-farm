...
import MercuryIcon from '@/assets/device-hub.svg?react'
...
type DynamicLogoProps = {
  ...
  height?: number
  className?: string
  logoType?: 'mercuryDeviceFarm' | 'emulatorHub'
}

export const DynamicLogo = memo(({ width, height, className, logoType = 'mercuryDeviceFarm' }: DynamicLogoProps) => {
  const colorScheme = useColorScheme()

  return (
    <>
      <ConditionalRender conditions={[logoType === 'mercuryDeviceFarm']}>
        <MercuryIcon
          className={cn(className, { [styles.darkLogo]: colorScheme === 'dark' })}
          height={height}
          title='Mercury Device Farm'
          width={width}
        />
      </ConditionalRender>
