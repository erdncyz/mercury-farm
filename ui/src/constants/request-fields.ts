export const DEVICE_LIST_FIELDS =
  'abi,battery.health,battery.level,battery.scale,battery.source,battery.status,battery.temp,bookedAt,bookedBefore,bookingSource,browser.apps,channel,cpuPlatform,createdAt,display.width,display.height,display.rotation,display.url,group,ios,macAddress,manufacturer,marketName,model,name,network.type,network.subtype,notes,openGLESVersion,operator,owner.email,owner.name,phone.iccid,phone.imei,phone.imsi,phone.phoneNumber,place,platform,present,product,provider.name,ready,releaseAt,sdk,serial,service,status,statusChangedAt,storageId,usage,using,version'
