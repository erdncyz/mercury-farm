...
import {ConnectGetForwardUrlMessage} from '../wire/wire.js'
...
export const MERCURY_ADMIN_EMAIL = process.env.MERCURY_ADMIN_EMAIL || 'mercury@test.com'
...
export const respond = function(res, code, message, data) {
    ...
    log.info('Responding with %s %s %s', code, message, truncateString(JSON.stringify(data) + '', 600))
...
export const filterDevice = function(req, device) {
    ...
    if (req.headers.is_generator === '1') {
        ...
        req.options.push.send([
            device.channel,
            wireutil.transaction(responseChannel, ConnectGetForwardUrlMessage.create())
        ])
    }
    if (fields) {
