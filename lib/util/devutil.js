...
export default syrup.serial()
    ...
    .define(function(options, properties, adb) {
        ...
        const devutil = Object.create(null)

        devutil.executeShellCommand = function(command) {
            return adb.getDevice(options.serial).execOut(command)
                .then(result => {
                    log.debug(`executing shell command ${command}, %s`, result)
                })
                .catch(err => {
                    if (/Unknown package:/i.test(err?.message || String(err))) {
                        log.info('Shell command skipped; package is already absent: %s', command)
        ...
        devutil.waitForLocalSocket = (sock, timeout = 60 * 1000) => new Promise(async(resolve, reject) => {
        ...
        devutil.makeIdentity = async function() {
            ...
            let customMarketName = properties['debug.mercury.product.device']
            ...
            var deviceName = process.env.MERCURY_PROVIDER_DEVICE_NAME
