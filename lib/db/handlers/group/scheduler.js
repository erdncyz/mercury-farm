...
import BuildModel from '../../models/build/index.js'
import {isDeviceActiveForBuild} from '../../models/build/model.js'
...
export default class GroupsScheduler {
    ...
    setScheduledTask({groupId, type, time = 0, job}) {
        if (Date.now() >= time) {
            return this.processTask({groupId, type, time, job})
        }
    ...
    processGroup(group) {
        ...
        if (now >= lastWindow.stop) {
            this.setScheduledTask({
                ...
                job: (group) => isOrigin ?
                    ...
                    this.deleteGroupWhenAutomationInactive(group)
            })
            return
        }
        ...
        this.setScheduledTask({
            ...
            ) ? this.deleteGroupWhenAutomationInactive(group) : this.doBecomeUnactiveGroup(group)
        })
    }
    ...
    async deleteGroupWhenAutomationInactive(group) {
        const [devices, build] = await Promise.all([
            dbapi.loadDevicesBySerials(group.devices || []),
            BuildModel.getBuild(group.id)
        ])
        const now = Date.now()
        const hasActiveAutomation = Boolean(build) && devices.some(device =>
            isDeviceActiveForBuild(device, group.id, now)
        )
        ...
        // The original stop time is an inactivity lease. ADB/WDA traffic can
        // keep that lease alive, so retain the transient group until the
        // provider emits LeaveGroupMessage after activity actually stops.
        this.setScheduledTask({
            groupId: group.id,
            type: 'WAIT_FOR_AUTOMATION_INACTIVITY',
            time: Date.now() + apiutil.ONE_MN,
            job: activeGroup => this.deleteGroupWhenAutomationInactive(activeGroup)
        })
