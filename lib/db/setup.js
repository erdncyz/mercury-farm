...
export default async function setupDb(conn) {
    ...
    await Promise.all(Object.entries(tables).map(([table, options]) => createTable(table, options)));
    return conn;
}
