...
export const GLOBAL_RUNTIME_SETTINGS_KEYS = ['androidRuntimeSettings', 'iosRuntimeSettings']

// Device runtime settings are chosen by admins and shared by every user, so they
// live on the MERCURY_ADMIN_EMAIL user next to the global alert message.
export const loadGlobalRuntimeSettings = async function() {
    const adminUser = await loadUser(apiutil.MERCURY_ADMIN_EMAIL)
    const settings = adminUser?.settings || {}
    return Object.fromEntries(GLOBAL_RUNTIME_SETTINGS_KEYS
        .filter((key) => settings[key] && typeof settings[key] === 'object')
...
export const updateUsersAlertMessage = async function(alertMessage) {
    const oldUser = await db.users.findOne({email: apiutil.MERCURY_ADMIN_EMAIL})
    const newUser = await db.users.findOneAndUpdate(
        {
            email: apiutil.MERCURY_ADMIN_EMAIL
        }
        , {
...
export const resetUserSettings = async function(email) {
    let settings = {}
    if (email === apiutil.MERCURY_ADMIN_EMAIL) {
        // Keep the shared device runtime settings; they are not personal preferences.
        settings = await loadGlobalRuntimeSettings()
    }
    return db.users.updateOne({email: email},
        {
            $set: {
