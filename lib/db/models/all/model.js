...
export const createBootStrap = function(env) {
    ...
    return GroupModel.createGroup({
        name: env.MERCURY_ROOT_GROUP_NAME,
        owner: {
            email: env.MERCURY_ADMIN_EMAIL,
            name: env.MERCURY_ADMIN_NAME
        },
        users: [env.MERCURY_ADMIN_EMAIL],
...
export const saveIosDeviceInitialState = function(publicIp, message) {
    const data = {
        ...
        'group.owner.email': process.env.MERCURY_ADMIN_EMAIL || 'mercury@test.com',
        'group.owner.name': process.env.MERCURY_ADMIN_NAME || 'Mercury',
        manufacturer: 'Apple'
    }
...
export const enhanceStatusChangedAt = function(serial, timeout, source = 'manual') {
    return db.devices.updateOne(
        {serial: serial},
        {
            $set: {
                bookedBefore: timeout,
                bookedAt: timeout > 0 ? getNow() : null,
                bookingSource: timeout > 0 ? source : null
            }
        }
    )
}
        ...
        {serial: serial},
        {
            $set: {
                ...
                ... typeof timeout === 'number' && {
                    ...
                    bookedBefore: timeout,
                    bookedAt: timeout > 0 ? getNow() : null,
                    bookingSource: timeout > 0 ? 'session' : null
                },
            }
        }
    )
...
export const setDeviceRotation = function(message) {
    ...
    // RotationEvent width/height are optional and usually absent (undefined).
    // Only persist them when they carry real dimensions; writing undefined
    // would be serialized as null by the MongoDB driver and wipe the stored
    // display size, which silently breaks touch input in the UI.
...
export const setDeviceDisplayName = function(serial, displayName) {
    const normalizedDisplayName = typeof displayName === 'string' ? displayName.trim() : ''
    const update = normalizedDisplayName ?
        {$set: {displayName: normalizedDisplayName}} :
        {$unset: {displayName: ''}}

    return db.devices.updateOne({serial: serial}, update)
}
...
// The `type` filter used to be matched against a `type` field that does not
// exist on device documents, so any capture request using it returned zero
// devices. Translate it to the fields that actually describe the platform.
const deviceTypeCondition = function(type) {
        ...
        return {
            $and: [
                ...
                {$or: [
                    ...
                    {platform: {$eq: null}},
                    {platform: {$regex: '^android$', $options: 'i'}}
                ]}
            ]
        }
    }
    if (value === 'ios' || value === 'tvos') {
        return {
            $or: [
                {platform: {$regex: '^(ios|tvos)$', $options: 'i'}},
                {manufacturer: {$eq: 'Apple'}},
                {ios: {$eq: true}}
            ]
        }
    }
    const exact = {$regex: `^${escapeRegExp(type)}$`, $options: 'i'}
    return {$or: [{platform: exact}, {deviceType: exact}]}
}
...
export const loadBookableDevicesWithFiltersLock = function(groups, filters, devicesFunc, limit = null) {
    ...
    if (filters && typeof filters === 'object') {
        Object.entries(filters).forEach(([field, condition]) => {
            if (condition !== null && condition !== undefined) {
                if (field === 'type') {
                    // Platform filter needs a translation, see deviceTypeCondition.
                    filterOptions.push(deviceTypeCondition(condition))
                }
    ...
    return db.devices.aggregate(pipeline).toArray()
        .then(devices => {
            ...
            return lockDevices(serials).then(() => {
...
export const loadOwnedDeviceChannels = async function() {
    const devices = await db.devices
        .find({owner: {$ne: null}, present: true, channel: {$exists: true, $ne: null}}, {projection: {channel: 1}})
        .toArray()
    return devices.map((device) => device.channel)
}

// dbapi.setDeviceType = function(serial, type) {
export const setDeviceType = async function(serial, type) {
    const device = await db.devices.findOne({serial: serial})
    const currentModel = device?.model || ''
    const currentMarketName = device?.marketName || ''
    const isIosManufacturer = device?.manufacturer === 'Apple'
    const looksLikePlaceholder =
        !currentModel ||
        !currentMarketName ||
        currentModel.startsWith('iOS Device ') ||
        currentMarketName.startsWith('iOS Device ') ||
        currentModel === serial ||
        currentMarketName === serial

    const setObj = {
        deviceType: type
    }

    if (isIosManufacturer && looksLikePlaceholder) {
        setObj.model = type
        setObj.marketName = type
    }
