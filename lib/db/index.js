...
const options = {
    ...
    // MongoDB container.
    get url() {
        const envVal = process.env.MONGODB_PORT_27017_TCP;
...
const handlers = [GroupChangeHandler, UserChangeHandler];
export default class DbClient {
    ...
    static async connect(opts = {}) {
        ...
        if (opts.push && opts.pushdev && opts.channelRouter) {
            for (const changeHandler of handlers) {
                if (!changeHandler.isPrepared) {
                    await changeHandler.init(opts.push, opts.pushdev, opts.channelRouter);
                }
            }
        }
        ...
        process.stderr.write(`\n*** DB_URL_DEBUG: "${resolvedUrl}" ***\n`);
        const records = await srv.resolve(resolvedUrl); // why?
        if (!records.shift()) {
    ...
    static async createZMQSockets({ sub, subdev, push, pushdev, channelRouter, }, _log = log) {
        ...
        if (sub && Array.isArray(sub)) {
            ...
            await Promise.all(sub.map(async (endpoint) => {
                try {
                    srv.attempt(await srv.resolve(endpoint), async (record) => {
        ...
        if (Array.isArray(push)) {
            ...
            await Promise.all(push.map(async (endpoint) => {
                try {
                    srv.attempt(await srv.resolve(endpoint), async (record) => {
        ...
        if (Array.isArray(pushdev)) {
            ...
            await Promise.all(pushdev.map(async (endpoint) => {
                try {
                    srv.attempt(await srv.resolve(endpoint), async (record) => {
                        _log.info('Sending output to "%s"', record.url);
                    ...
                    _log.fatal('Unable to connect to pushdev endpoint: %s', err);
                    lifecycle.fatal();
                }
            }));
            pushdev = _pushdev;
    ...
    static ensureConnectivity = (fn) => async (...args) => {
