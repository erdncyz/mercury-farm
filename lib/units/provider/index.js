...
import ADBObserver from './ADBObserver.js';
import { DeviceRegisteredMessage, DeviceAbsentMessage, DeviceIntroductionMessage, ProviderMessage, DeviceStatusMessage } from '../../wire/wire.js';
...
export default async (options) => {
    ...
    const portPool = new ResourcePool(options.ports);
    ...
    const stats = (twice = true) => {
        ...
        log.info(`Providing ${processStats.running.length} of ${processStats.total} device(s); starting: [${processStats.starting.join(', ')}], waiting: [${processStats.waiting.join(', ')}]`);
    ...
    const filterDevice = (listener) => ((device, oldType) => {
    ...
    const register = (device) => new Promise(async (resolve, reject) => {
        push.send([
            wireutil.global,
    ...
    const startDeviceWork = async (device, restart = false, reregister = false) => {
    ...
    tracker.on('stuck', async (device, health) => {
        log.warn('Device %s is stuck [attempts: %s, first_healthcheck: %s, last_healthcheck: %s]', device.serial, new Date(health.firstFailureTime).toISOString(), new Date(health.lastAttemptTime).toISOString());
        if (!processManager.has(device.serial)) {
    ...
    tracker.on('disconnect', filterDevice(async (device) => {
        ...
        if (!processManager.has(device.serial)) {
            log.warn('Device is disconnected, but process is not running %s', device.isStuck ? '[Device got stuck earlier]' : '');
            return;
        }
        if (!device.isStuck) {
            removeDevice(device);
