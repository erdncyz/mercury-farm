...
import { DeviceRegisteredMessage, DeviceAbsentMessage, DeviceStatusMessage, DeviceIosIntroductionMessage, ProviderIosMessage } from '../../wire/wire.js';
...
export default async (options) => {
    ...
    let espTimer;
    let usbWatchdogTimer;
    const missingUsbChecks = new Map();
    const resolveUdid = (device) => {
        if (!device)
            return '';
    ...
    const espObserver = async () => {
        ...
        diffAdd.forEach((dev) => {
            log.info(`Added ESP32 to the pool. path=%s, productId=%s, manufacturer=%s`, dev.path, dev.productId, dev.manufacturer);
        });
        diffRemove.forEach((dev) => {
            log.info(`Removed ESP32 from the pool. path=%s, productId=%s, manufacturer=%s`, dev.path, dev.productId, dev.manufacturer);
        });
        curEsp32 = newDevices;
    ...
    const portPool = new ResourcePool(options.wdaPorts.map((wdaPort, i) => ({
    ...
    const stats = (twice = true) => {
        ...
        log.info(`Providing ${processStats.running.length} of ${processStats.total} iOS device(s); starting: [${processStats.starting.join(', ')}], waiting: [${processStats.waiting.join(', ')}]`);
        if (twice) {
    ...
    const filterDevice = (listener) => ((rawDevice, simulator) => {
    ...
    const register = (udid) => new Promise(async (resolve, reject) => {
    ...
    const startDeviceWork = async (udid, restart = false, reRegister = false) => {
    ...
    const onAttach = filterDevice(async (udid, isSimulator) => {
    ...
    const onDetach = filterDevice((udid) => {
        missingUsbChecks.delete(udid);
    ...
    const usbDetachWatchdog = () => {
        try {
            const connectedPhysicalDevices = new Set();
            ...
            managedIds.forEach((udid) => {
                ...
                if (missingCount >= USB_MISSING_THRESHOLD) {
                    log.warn('Device "%s" missing from usbmux for %s checks. Forcing detach.', udid, missingCount);
                    processManager.clearTimer(udid);
                    removeDevice(udid);
                    missingUsbChecks.delete(udid);
                }
    ...
    lifecycle.observe(async () => {
        ...
        [push, sub].forEach((sock) => sock.close());
    });
};
