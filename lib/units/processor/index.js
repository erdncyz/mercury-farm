...
export default db.ensureConnectivity(async (options) => {
    ...
    await Promise.all(options.endpoints.appDealer.map(async (endpoint) => {
        try {
            return await srv.resolve(endpoint).then((records) => srv.attempt(records, (record) => {
    ...
    await Promise.all(options.endpoints.devDealer.map(async (endpoint) => {
        try {
            return await srv.resolve(endpoint).then((records) => srv.attempt(records, (record) => {
    ...
    const router = new WireRouter()
        ...
        await Promise.all([
            ...
            dbapi.sendEvent(`device_${message.usage || 'use'}`, {}, { deviceSerial: message.serial, userEmail: message.owner.email, groupId: message.owner.group }, Date.now())
        ]);
        appDealer.send([channel, data]);
    })
        ...
        if (message.owner.group) {
            buildIds.add(message.owner.group);
        }
        if (device?.group?.id) {
        ...
        await dbapi.sendEvent('device_leave', {}, { deviceSerial: message.serial, userEmail: message.owner.email, groupId: message.owner.group }, Date.now());
        await Promise.all([...buildIds].map(buildId => BuildModel.finishBuildIfInactive(buildId)));
        appDealer.send([channel, data]);
    })
        ...
        .on(UpdateIosDevice, (channel, message, data) => dbapi.updateIosDevice(message))
