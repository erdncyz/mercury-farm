...
export default (function(options) {
    ...
    function tryConnect(times, delay) {
        openid.Issuer.discover(options.openid.identifierUrl).then((keycloakIssuer) => {
            ...
            if (/outgoing request timed out/.test(err.message) && times > 1) {
                new Promise(resolve => setTimeout(resolve, delay))
                    .then(() => {
                        log.info('Retrying connect to openid-client')
                        tryConnect(times - 1, delay)
                    })
                return
            }
    ...
    log.info('Connecting to openid-client')
    tryConnect(3, 500)

    app.use(function(req, res, next) {
        res.setHeader('X-mercury-unit', 'auth-openid')
        next()
    })
