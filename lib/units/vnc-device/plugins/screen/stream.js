...
import {GestureStartMessage, GestureStopMessage, TouchDownMessage, TouchMoveMessage, TouchUpMessage, TouchCommitMessage, TouchResetMessage, TypeMessage, KeyDownMessage, KeyUpMessage} from '../../../../wire/wire.js'
...
export default syrup.serial()
    ...
    .define(function(options, solo, notifier, push, router, group) {
        ...
        wss.on('connection', async function connection(ws) {
            ...
            router
                .on(GestureStartMessage, function(channel, message) {
                })
                .on(GestureStopMessage, function(channel, message) {
                })
                .on(TouchDownMessage, function(channel, message) {
                    lastClicked.x = message.x * height
                    lastClicked.y = message.y * width
                    r.pointerEvent(message.x * height, message.y * width, 1)
                })
                .on(TouchMoveMessage, function(channel, message) {
                })
                .on(TouchUpMessage, function(channel, message) {
                })
                .on(TouchCommitMessage, function(channel, message) {
                })
                .on(TouchResetMessage, function(channel, message) {
                })
                .on(TypeMessage, function(channel, message) {
                    let keyCode = message.text.charCodeAt(0)
                    r.keyEvent(keyCode, 1)
                    r.keyEvent(keyCode, 0)
                    r.requestUpdate(false, 0, 0, r.width, r.height)
                })
                .on(KeyDownMessage, function(channel, message) {
                    r.keyEvent(keyNameToX11KeyCode(message.key), 1)
                    r.requestUpdate(false, 0, 0, r.width, r.height)
                })
                .on(KeyUpMessage, function(channel, message) {
