...
import { PhysicalIdentifyMessage, KeyDownMessage, KeyUpMessage, KeyPressMessage, TypeMessage, RotateMessage, UnlockDeviceMessage } from "../../../wire/wire.js";
class Deferred {
    promise;
    resolve;
    reject;
    timeout = setTimeout(this.reject || (() => { }), GRPC_WAIT_TIMEOUT, new Error('Timeout'));
    constructor() {
...
export default syrup.serial()
    ...
    const runSingleRelaunch = (fn) => {
        if (relaunchInFlight) {
            log.warn('Relaunch already in progress, skipping duplicate trigger');
    ...
    const stopAgent = () => devutil.killProcsByComm('mercury.agent', 'mercury.agent');
    const callService = async (intent) => {
    ...
    const handleEnvelope = (data) => {
        const envelope = apk.wire.Envelope.decode(data);
        if (envelope.id) {
    ...
    const runAgentCommand = (type, cmd) => agent.writer?.write(new apk.wire.Envelope(null, type, cmd.encodeNB()).encodeNB());
    const runServiceCommand = (type, cmd) => {
    ...
    const keyEvent = (data) => runAgentCommand(apk.wire.MessageType.DO_KEYEVENT, new apk.wire.KeyEventRequest(data));
    const plugin = new class extends EventEmitter {
        type = (text) => devutil.executeShellCommand("am broadcast -a ADB_INPUT_TEXT --es msg '" + text + "'");
        paste = async (text) => {
        ...
        getDisplay = (id) => runServiceCommand(apk.wire.MessageType.GET_DISPLAY, new apk.wire.GetDisplayRequest(id))
            .then((data) => {
        ...
        wake = () => runAgentCommand(apk.wire.MessageType.DO_WAKE, new apk.wire.DoWakeRequest());
        rotate = (rotation) => runAgentCommand(apk.wire.MessageType.SET_ROTATION, new apk.wire.SetRotationRequest(rotation, true));
        freezeRotation = (rotation) => runAgentCommand(apk.wire.MessageType.SET_ROTATION, new apk.wire.SetRotationRequest(rotation, true));
        thawRotation = () => runAgentCommand(apk.wire.MessageType.SET_ROTATION, new apk.wire.SetRotationRequest(0, false));
        version = () => runServiceCommand(apk.wire.MessageType.GET_VERSION, new apk.wire.GetVersionRequest())
            .then((data) => {
        ...
        unlock = () => runServiceCommand(apk.wire.MessageType.SET_KEYGUARD_STATE, new apk.wire.SetKeyguardStateRequest(false))
            .then((data) => {
        ...
        lock = () => runServiceCommand(apk.wire.MessageType.SET_KEYGUARD_STATE, new apk.wire.SetKeyguardStateRequest(true))
            .then((data) => {
            const response = apk.wire.SetKeyguardStateResponse.decode(data);
            if (!response.success) {
        ...
        acquireWakeLock = () => runServiceCommand(apk.wire.MessageType.SET_WAKE_LOCK, new apk.wire.SetWakeLockRequest(true))
            .then((data) => {
        ...
        releaseWakeLock = () => runServiceCommand(apk.wire.MessageType.SET_WAKE_LOCK, new apk.wire.SetWakeLockRequest(false))
            .then((data) => {
            const response = apk.wire.SetWakeLockResponse.decode(data);
            if (!response.success) {
        ...
        setClipboard = (text) => runServiceCommand(apk.wire.MessageType.SET_CLIPBOARD, new apk.wire.SetClipboardRequest(apk.wire.ClipboardType.TEXT, text))
            .then((data) => {
        ...
        getClipboard = () => runServiceCommand(apk.wire.MessageType.GET_CLIPBOARD, new apk.wire.GetClipboardRequest(apk.wire.ClipboardType.TEXT))
            .then((data) => {
        ...
        getBrowsers = () => runServiceCommand(apk.wire.MessageType.GET_BROWSERS, new apk.wire.GetBrowsersRequest())
            .then((data) => {
        ...
        getMobileServices = () => runServiceCommand(apk.wire.MessageType.GET_SERVICES, new apk.wire.GetServicesAvailabilityRequest())
            .then((data) => {
        ...
        getProperties = (properties) => runServiceCommand(apk.wire.MessageType.GET_PROPERTIES, new apk.wire.GetPropertiesRequest(properties))
            .then(async (data) => {
            const response = apk.wire.GetPropertiesResponse.decode(data);
            if (!response.success) {
                throw new Error('Unable to get properties');
            }
            const mapped = response.properties.reduce((acc, property) => {
        ...
        getAccounts = (data) => runServiceCommand(apk.wire.MessageType.GET_ACCOUNTS, new apk.wire.GetAccountsRequest({ type: data.type }))
            .then((data) => {
        ...
        addAccountMenu = () => runServiceCommand(apk.wire.MessageType.DO_ADD_ACCOUNT_MENU, new apk.wire.DoAddAccountMenuRequest())
            .then((data) => {
        ...
        cleanupBondedBluetoothDevices = () => runServiceCommand(apk.wire.MessageType.DO_CLEAN_BLUETOOTH_BONDED_DEVICES, new apk.wire.DoCleanBluetoothBondedDevicesRequest())
            .then((data) => {
        ...
        setRingerMode = (mode) => runServiceCommand(apk.wire.MessageType.SET_RINGER_MODE, new apk.wire.SetRingerModeRequest(mode))
            .then((data) => {
        ...
        getRingerMode = () => runServiceCommand(apk.wire.MessageType.GET_RINGER_MODE, new apk.wire.GetRingerModeRequest())
            .then((data) => {
        ...
        setWifiEnabled = (enabled) => runServiceCommand(apk.wire.MessageType.SET_WIFI_ENABLED, new apk.wire.SetWifiEnabledRequest(enabled))
            .then((data) => {
        ...
        getWifiStatus = () => runServiceCommand(apk.wire.MessageType.GET_WIFI_STATUS, new apk.wire.GetWifiStatusRequest())
            .then((data) => {
        ...
        setBluetoothEnabled = (enabled) => runServiceCommand(apk.wire.MessageType.SET_BLUETOOTH_ENABLED, new apk.wire.SetBluetoothEnabledRequest(enabled))
            .then((data) => {
        ...
        cleanBluetoothBonds = () => runServiceCommand(apk.wire.MessageType.DO_CLEAN_BLUETOOTH_BONDED_DEVICES, new apk.wire.DoCleanBluetoothBondedDevicesRequest())
            .then((data) => {
            const response = apk.wire.DoCleanBluetoothBondedDevicesResponse.decode(data);
            if (!response.success) {
                throw new Error('Unable to clean Bluetooth bonded devices');
        ...
        getBluetoothStatus = () => runServiceCommand(apk.wire.MessageType.GET_BLUETOOTH_STATUS, new apk.wire.GetBluetoothStatusRequest())
            .then((data) => {
        ...
        getSdStatus = () => runServiceCommand(apk.wire.MessageType.GET_SD_STATUS, new apk.wire.GetSdStatusRequest())
            .then((data) => {
        ...
        setMasterMute = (enabled) => runServiceCommand(apk.wire.MessageType.SET_MASTER_MUTE, new apk.wire.SetMasterMuteRequest(enabled))
            .then((data) => {
    ...
    const waitForAgentDeath = (conn, ms = 3000) => new Promise(resolve => {
        let timeout = setTimeout(resolve, ms);
        conn.once('end', async () => {
            clearTimeout(timeout);
            runSingleRelaunch(async () => {
    ...
    const onServiceDeath = async () => {
        return runSingleRelaunch(async () => {
            try {
                const startTime = Date.now();
                log.important('Service connection ended, attempting to relaunch');
                service.reader?.removeAllListeners();
                service.reader = null;
                service.writer = null;
                service.socket = null;
                agent.writer = null;
    ...
    const openService = async () => {
        ...
        let serviceDeathHandled = false;
        const handleServiceDeath = () => {
            if (serviceDeathHandled) {
                return;
            }
            serviceDeathHandled = true;
            void onServiceDeath();
        };
        service.socket.once('end', handleServiceDeath);
        service.socket.once('error', handleServiceDeath);
    ...
    router
        ...
        .on(TypeMessage, (channel, message) => plugin.type(message.text))
        .on(RotateMessage, (channel, message) => plugin.rotate(message.rotation))
        .on(UnlockDeviceMessage, (channel, message) => plugin.unlockDevice());
    return plugin;
});
