...
import {LogcatApplyFiltersMessage, LogcatStartMessage, LogcatStopMessage} from '../../../wire/wire.js'
export default syrup.serial()
    ...
    .define(function(options, adb, router, push, group) {
        ...
        router
            .on(LogcatStartMessage, function(channel, message) {
            ...
            .on(LogcatApplyFiltersMessage, function(channel, message) {
                var reply = wireutil.reply(options.serial)
            ...
            .on(LogcatStopMessage, function(channel) {
                var reply = wireutil.reply(options.serial)
                plugin.stop()
                    .then(function() {
                        push.send([
