...
import {ForwardCreateMessage, ForwardRemoveMessage, ForwardTestMessage} from '../../../../wire/wire.js'
export default syrup.serial()
    ...
    .define(async function(options, adb, router, push, minirev, group) {
        ...
        var serviceOutput = null
        var serviceStarting = null
        function startService() {
            if (serviceStarting) {
                return serviceStarting
            }
            log.info('Launching reverse port forwarding service')
            serviceStarting = adb.getDevice(options.serial).shell([
                'exec',
                minirev.bin
            ])
                .then(function(out) {
                    ...
                    out.once('end', function() {
                        log.warn('Forward shell ended; reverse port forwarding will be restarted on demand')
                        serviceOutput = null
                    })
                    out.once('error', function(err) {
                        log.warn('Forward shell had an error; reverse port forwarding will be restarted on demand: %s', err?.message || err)
                        serviceOutput = null
                    })
        ...
        await ensureService()
            .then(function() {
                router
                    .on(ForwardTestMessage, function(channel, message) {
                    ...
                    .on(ForwardCreateMessage, function(channel, message) {
                        var reply = wireutil.reply(options.serial)
                    ...
                    .on(ForwardRemoveMessage, function(channel, message) {
                        var reply = wireutil.reply(options.serial)
