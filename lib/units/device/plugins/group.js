...
export default syrup.serial()
    ...
    group.setCheckBeforeAction(async (message) => !message.requirements || grouputil.match(ident, message.requirements));
    group.on('join', () => {
    ...
    group.on('leave', async () => {
        try {
            if (options.screenReset) {
                ...
                const { isInOrigin } = await runTransactionDev(wireutil.global, DeviceGetIsInOrigin, { serial: options.serial }, { sub, push, router });
                if (isInOrigin) {
