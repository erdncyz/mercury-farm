...
export default class AndroidWebRtcTransport {
    constructor(options, scrcpy) {
        this.options = options
        this.scrcpy = scrcpy
        this.log = logger.createLogger('device:plugins:screen:webrtc')
        this.peers = new Map()
    ...
    async handleMessage(id, ws, message) {
        ...
        if (signal.type === 'webrtc_ice') {
            ...
            if (peer) {
                await peer.pc.addIceCandidate(signal.candidate || null)
            }
            else {
    ...
    async createPeer(id, ws, signal) {
        ...
        const pc = new RTCPeerConnection({
            ...
            iceServers: this.iceServers,
            icePortRange: [
                Number(this.options.screenWebrtcPortMin) || 13000,
                Number(this.options.screenWebrtcPortMax) || 13100
            ],
            ...(this.options.screenWebrtcPublicIp ?
                {iceAdditionalHostAddresses: [this.options.screenWebrtcPublicIp]} :
                {})
        })
        const track = new MediaStreamTrack({kind: 'video', codec})
        const mediaStream = new MediaStream({id: `android-${this.options.serial}`})
        ...
        try {
            ...
            const pendingCandidates = this.pendingIceCandidates.get(id) || []
            this.pendingIceCandidates.delete(id)
    ...
    async ensureCapture() {
        ...
        this.captureStart = this.capture.start().then(info => {
            this.info = info
            this.log.info('Android H.264 capture started for %s at %dx%d', this.options.serial, info.width, info.height)
            return info
        })
        return this.captureStart
    }
    ...
    onCaptureError(error) {
        ...
        this.log.error('Android H.264 capture failed for %s: %s', this.options.serial, error.message)
        for (const peer of this.peers.values()) {
