...
export default class AndroidWebRtcTransport {
    constructor(options, scrcpy, transportOptions = {}) {
        ...
        this.webSocketNeedsKeyframe = new WeakMap()
    ...
    // ICE port window makes consecutive sessions bind ports that were not
    // recently released, so negotiation does not hit a dead flow. Keep the
    // windows disjoint: returning [start, max] would still let werift reuse a
    // port from every earlier (overlapping) window.
    allocateIcePortRange() {
        ...
        const portCount = max - min + 1
        if (portCount <= ICE_PORT_ROTATION_STEP) {
            return [min, max]
        }

        const windows = Math.ceil(portCount / ICE_PORT_ROTATION_STEP)
        const start = min + (this.portRotation % windows) * ICE_PORT_ROTATION_STEP
        const end = Math.min(max, start + ICE_PORT_ROTATION_STEP - 1)
        this.portRotation += 1
        return [start, end]
    }
    ...
    async addWebSocketClient(id, ws) {
        ...
        this.webSocketNeedsKeyframe.set(ws, false)
    ...
    sendWebSocketPacket(ws, packet) {
        ...
        if (ws.bufferedAmount > MAX_WEBSOCKET_BUFFERED_BYTES) {
            this.webSocketNeedsKeyframe.set(ws, true)
            return
        }
        if (this.webSocketNeedsKeyframe.get(ws) && !packet.config && !packet.keyframe) {
            return
        }
        if (packet.keyframe) {
            this.webSocketNeedsKeyframe.set(ws, false)
    ...
    // Takes effect on the next capture start. An idle capture that is only
    // waiting for its scheduled stop is dropped so it cannot mask the change.
    updateCaptureOptions(patch) {
        this.captureOptions = {...this.captureOptions, ...patch}
        if (this.capture && this.peers.size === 0 && this.webSocketClients.size === 0) {
            this.stopCapture()
        }
    }
