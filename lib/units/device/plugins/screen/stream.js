...
import AndroidWebRtcTransport from './webrtc.js'
import display from '../util/display.js'
import options from './options.js'
import group from '../group.js'
import * as jwtutil from '../../../../util/jwtutil.js'
import {NoGroupError} from '../../../../util/grouputil.js'
import {ApplyAndroidRuntimeSettingsMessage, ChangeQualityMessage} from '../../../../wire/wire.js'
export default syrup.serial()
    ...
    .define(function(options, adb, router, minicap, scrcpy, display, screenOptions, group) {
        ...
        function FrameProducer(config, grabber) {
            ...
            this.currentFrameRate = Number(options.screenFrameRate) || 0
            ...
            this.on('start', this._startWatchdog.bind(this))
            this.on('stop', this._stopWatchdog.bind(this))
        }
        ...
        // Watchdog: detect silent minicap freezes (socket open but no frames).
        // Checked periodically; only acts while clients are connected so that a
        // genuinely static screen (minicap idle on purpose) is not mistaken for
        // a freeze too aggressively.
        FrameProducer.WATCHDOG_CHECK_INTERVAL = 5000
        FrameProducer.WATCHDOG_FRAME_TIMEOUT = 30000
        ...
        FrameProducer.prototype.changeFrameRate = function(newFrameRate) {
            if (this.currentFrameRate === newFrameRate) {
                log.info('Keeping %d as current frame producer frame rate', newFrameRate)
                return
            }
            log.info('Setting frame producer frame rate to %d', newFrameRate)
            this.currentFrameRate = newFrameRate
            this._configChanged()
        }
        FrameProducer.prototype.applyAndroidRuntimeSettings = function(message) {
            const nextFrameRate = Number(message?.screenFrameRate) || 0
            const nextQuality = Number(message?.screenJpegQuality) || 0
            let changed = false

            if (nextFrameRate > 0 && this.currentFrameRate !== nextFrameRate) {
                this.currentFrameRate = nextFrameRate
                changed = true
            }

            if (nextQuality > 0 && this.frameConfig.quality !== nextQuality) {
                this.frameConfig.quality = nextQuality
                changed = true
            }

            if (changed) {
                log.info('Applying Android runtime settings: frameRate=%d quality=%d', this.currentFrameRate, this.frameConfig.quality)
                this._configChanged()
            }
            else {
                log.info('Android runtime settings unchanged: frameRate=%d quality=%d', this.currentFrameRate, this.frameConfig.quality)
            }
        }
        FrameProducer.prototype.restoreDefaultSettings = function() {
            const defaultFrameRate = Number(options.screenFrameRate) || 0
            const defaultQuality = Number(options.screenJpegQuality) || 0
            let changed = false

            if (defaultFrameRate > 0 && this.currentFrameRate !== defaultFrameRate) {
                this.currentFrameRate = defaultFrameRate
                changed = true
            }

            if (defaultQuality > 0 && this.frameConfig.quality !== defaultQuality) {
                this.frameConfig.quality = defaultQuality
                changed = true
            }

            if (changed) {
                log.info('Restoring default screen settings: frameRate=%d quality=%d', this.currentFrameRate, this.frameConfig.quality)
                this._configChanged()
            }
        }
        ...
        FrameProducer.prototype._startWatchdog = function() {
            // Scrcpy delivers frames through a separate path that doesn't go
            // through nextFrame(), so the frame-time watchdog only applies to
            // the minicap pipeline.
            if (options.needScrcpy) {
                return
            }
            this._stopWatchdog()
            ...
            this.watchdogTimer = setInterval(
                this._watchdogCheck.bind(this),
                FrameProducer.WATCHDOG_CHECK_INTERVAL
            )
        }
        FrameProducer.prototype._stopWatchdog = function() {
            if (this.watchdogTimer) {
                clearInterval(this.watchdogTimer)
                this.watchdogTimer = null
            }
        }
        FrameProducer.prototype._watchdogCheck = function() {
            if (this.runningState !== FrameProducer.STATE_STARTED || this.failed) {
                return
            }
            // No connected clients means no frames are expected; keep the
            // baseline fresh so we don't trigger right after someone connects.
            if (!this.broadcastSet || this.broadcastSet.keys().length === 0) {
                this.lastFrameTime = Date.now()
                return
            }
            var elapsed = Date.now() - this.lastFrameTime
            if (elapsed >= FrameProducer.WATCHDOG_FRAME_TIMEOUT) {
                log.warn(
                    'No frames for %dms while clients connected; restarting frame producer (watchdog)',
                    elapsed
                )
                this.failCounter.inc()
                this.lastFrameTime = Date.now()
        ...
        FrameProducer.prototype._failLimitExceeded = function(limit, time) {
            this._stop()
            this.failed = true
            log.warn('Frame producer failed more than %d times in %dms, will attempt recovery in 30s', limit, time)
            this.recoveryTimeoutID = setTimeout(function() {
                this.recoveryTimeoutID = null
                log.info('Attempting frame producer recovery after cooldown')
                this.failed = false
                this.failCounter = new FailCounter(3, 10000)
                this.failCounter.on('exceedLimit', this._failLimitExceeded.bind(this))
                this.start()
        ...
        FrameProducer.prototype._startService = function() {
            ...
            if (this.currentFrameRate <= 0.0) {
                return minicap.run(this.grabber, util.format('-S -Q %d -P %s', this.frameConfig.quality, this.frameConfig.toString()))
                    .timeout(10000)
            }
            else {
                return minicap.run(this.grabber, util.format('-S -r %d -Q %d -P %s', this.currentFrameRate, this.frameConfig.quality, this.frameConfig.toString()))
                    .timeout(10000)
            }
        }
        ...
        return createServer()
            .then(function(wss) {
                ...
                var webRtcTransport = new AndroidWebRtcTransport(options, scrcpy)
                ...
                router.on(ChangeQualityMessage, function(channel, message) {
                    frameProducer.changeQuality(message.quality)
                })
                router.on(ApplyAndroidRuntimeSettingsMessage, function(channel, message) {
                    frameProducer.applyAndroidRuntimeSettings(message)
                })
                group.on('leave', function() {
                    frameProducer.restoreDefaultSettings()
                })
                ...
                frameProducer.on('readable', function next() {
                    ...
                    if (frame) {
                        broadcastSet.keys().forEach(function(id) {
                            var client = broadcastSet.get(id)
                            if (client) {
                                if (client.sending) {
                                    ...
                                    client.onFrame(frame)
                                        .then(function sendPending() {
                                            if (client.pendingFrame) {
                                                var nextFrame = client.pendingFrame
                                                client.pendingFrame = null
                                                return client.onFrame(nextFrame).then(sendPending)
                                            }
                ...
                wss.on('connection', async function(ws, req) {
                    ...
                    ws.on('message', async function(data, isBinary) {
                        ...
                        try {
                            ...
                            log.warn('WebRTC signaling failed for device %s: %s', options.serial, error.message)
                        ...
                        var match = /^(on|off|(size) ([0-9]+)x([0-9]+))$/.exec(message)
                        if (match) {
                    ...
                    ws.on('close', function() {
                        ...
                        webRtcTransport.removePeer(id).catch(error => {
                            log.warn('Failed to close WebRTC peer for %s: %s', options.serial, error.message)
                        })
