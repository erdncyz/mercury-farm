...
export function normalizeShellCommand(command) {
    return command.replace(/^\s*adb\s+shell\s+/, '')
}
...
export default syrup.serial()
    ...
    .define(function(options, adb, router, push, sub) {
        ...
        router.on(ShellCommandMessage, async function(channel, message) {
            ...
            const command = normalizeShellCommand(message.command)
            log.info('Running shell command "%s"', command)
            const stream = await adb.getDevice(options.serial).shell(command)
