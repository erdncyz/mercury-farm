...
import {AccountAddMenuMessage, AccountAddMessage, AccountCheckMessage, AccountGetMessage, AccountRemoveMessage} from '../../../wire/wire.js'
export default syrup.serial()
    ...
    .define(function(options, service, identity, touch, router, push, adb) {
        ...
        router.on(AccountCheckMessage, function(channel, message) {
        ...
        router.on(AccountGetMessage, function(channel, message) {
            var reply = wireutil.reply(options.serial)
        ...
        router.on(AccountRemoveMessage, function(channel, message) {
            var reply = wireutil.reply(options.serial)
        ...
        router.on(AccountAddMenuMessage, function(channel) {
            var reply = wireutil.reply(options.serial)
        ...
        router.on(AccountAddMessage, function(channel, message) {
            var reply = wireutil.reply(options.serial)
            var type = 'com.google'
            ...
            return checkAccount(type, account)
                ...
                .catch(function() {
                    return adb.getDevice(options.serial).clear('com.google.android.gsf.login')
                        .catch(function() {
                            // The package name is different in 2.3, so let's try the old name
                            // if the new name fails.
                            return adb.getDevice(options.serial).clear('com.google.android.gsf')
                        })
                        .then(function() {
