...
export default syrup.serial()
    ...
    .dependency(push)
    .dependency(storage)
    .define((options, adb, router, push, storage) => {
    const log = logger.createLogger('device:plugins:install');
    const reply = wireutil.reply(options.serial);
    const isUnknownPackageError = (err) => /Unknown package:/i.test(err?.message || String(err));
    router.on(InstallMessage, async (channel, message) => {
        ...
        const pushApp = async (channel) => {
            try {
                ...
                let transferError;
                transfer.on('error', (error) => transferError = error);
        ...
        const install = async (installCmd, attempt = 0) => {
            try {
        ...
        try {
            ...
            await promiseutil.periodicNotify(install(installCmd), 250);
        }
        catch (err) {
