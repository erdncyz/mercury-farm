...
import {GetInstalledApplications, KillDeviceApp, LaunchDeviceApp, TerminateDeviceApp} from '../../../wire/wire.js'
    ...
    .define((options, push, router, sdb, webinspector, group) => {
        ...
        const plugin = {
            ...
            start: async() => {
                ...
                router
                    .on(GetInstalledApplications, async(channel) =>
                        success(channel, Object.fromEntries(await sdb.getApps()))
                    )

                    .on(LaunchDeviceApp, plugin.launchApp)

                    .on(TerminateDeviceApp, async(channel) =>
                        success(channel, await plugin.killApp())
                    )
                    .on(KillDeviceApp, async(channel) =>
