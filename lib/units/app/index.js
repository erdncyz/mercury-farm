            ...
            if (req.path.endsWith('.js') && req.path.startsWith('/assets/')) {
                const reloadScript = `
                    ...
                    if(typeof window !== "undefined") {
                        ...
                        // Prevent infinite reload loops if asset is permanently unavailable.
                        if (url.searchParams.get("stale-reload") === "1") {
                            console.error("Stale chunk reload already attempted once. Stopping auto-reload loop.");
                        } else {
                            url.searchParams.set("stale-reload", "1");
