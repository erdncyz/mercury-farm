...
import { AutomationAliveMessage, AutoGroupMessage, DeviceStatusChange, GroupMessage, JoinGroupMessage, LeaveGroupMessage, UngroupMessage } from '../../../wire/wire.js';
...
export default syrup.serial()
    ...
    const plugin = new class GroupManager extends EventEmitter {
        currentGroup = null;
        currentUsage = null;
        aliveTimer = null;
        ...
        join = async (newGroup, timeout, usage, keys) => {
            try {
                if (!newGroup?.group) {
        ...
        beforeActionCheck = async (message) => true;
        // Set that for custom checks before GroupMessage/UngroupMessage processed (optional)
        setCheckBeforeAction = (cb) => {
            this.beforeActionCheck = cb;
        };
        checkBeforeAction = (msgName, message, channel, reply) => this.beforeActionCheck(message)
            .catch((err) => {
