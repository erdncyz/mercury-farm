...
import * as DeviceModel from '../../../db/models/device/model.js'
...
function getGenericDevices(req, res, loadDevices) {
    log.info('getGenericDevices: %s', JSON.stringify(req.user))
            ...
            const cb = device => {
                ...
                if (!adbPort) {
                    dbapi.initiallySetAdbPort(device.serial)
                        .then(port => {
                            if (port) {
                                log.info('Applied adb port %s for %s', port, device.serial)
                                device.adbPort = port
                            }
                            else {
                                log.warn('Cant apply port for %s', device.serial)
                            }
                            count++
                            cb(devices[count])
                        })
...
function removeDevice(serial, req, res) {
    const presentState = typeof req.query.present !== 'undefined' ? req.query.present === 'true' : undefined
    const bookingState = typeof req.query.booked !== 'undefined' ? req.query.booked === 'true' : undefined
    const notesState = typeof req.query.annotated !== 'undefined' ? req.query.annotated === 'true' : undefined
    const controllingState = typeof req.query.controlled !== 'undefined' ? req.query.controlled === 'true' : undefined
...
function getDeviceBySerial(req, res) {
    ...
    dbapi.loadDevice(req.user.groups.subscribed, serial)
        .then(async device => {
            // Active automation devices are globally watchable by authenticated
            // users. This only broadens the read endpoint; ownership checks on
            // control, booking and release operations remain unchanged.
            if (!device) {
                const automationDevice = await DeviceModel.loadDeviceBySerial(serial)
                if (automationDevice?.usage === 'automation') {
...
function getDeviceSize(req, res) {
    ...
    return dbapi.getDeviceDisplaySize(serial)
        ...
        .catch(function(err) {
            log.info('Failed to get device size: %s', err.stack)
...
function getDeviceOwner(req, res) {
    ...
    return dbapi.getDeviceGroupOwner(serial)
        ...
        .catch(function(err) {
            log.info('Failed to get device owner: %s', err.stack)
...
function getDeviceType(req, res) {
    ...
    return dbapi.getDeviceType(serial)
        ...
        .catch(function(err) {
            log.info('Failed to get device type: %s', err.stack)
...
function addOriginGroupDevices(req, res) {
    ...
    async function askUpdateDeviceOriginGroup(group, serial) {
        log.info('[addOriginGroupDevices] Ask to update device origin group [serial: "%s", group: "%s"]', serial, group.id)
        const signature = util.format('%s', uuidv4()).replace(/-/g, '')
        const result = await dbapi.updateDeviceOriginGroup(serial, group, signature)

        if (result) {
            log.info('[addOriginGroupDevices] Successfully updated device origin group [serial: "%s", group: "%s"]', serial, group.id)

            const device = await dbapi.loadDeviceBySerial(serial)
            if (fields) {
                return _.pick(apiutil.publishDevice(device, req), fields.split(','))
            }

            return apiutil.publishDevice(device, req)
        }
    }

    function updateDeviceOriginGroup(group, serial) {
        log.info('[addOriginGroupDevices] Add device %s to origin group %s', serial, group.id)
        const lock = {}
        return dbapi.lockDeviceByOrigin(req.user.groups.subscribed, serial).then(function(stats) {
            if (stats.modifiedCount === 0) {
                return apiutil.lightComputeStats(res, stats)
            ...
            return dbapi.isUpdateDeviceOriginGroupAllowed(serial, group)
                .then(function(updatingAllowed) {
                    if (!updatingAllowed) {
                        log.error('[addOriginGroupDevices] Device group editing is not allowed %s', serial, group.id)
    ...
    return dbapi.getUserGroup(req.user.email, req.params.id).then(function(group) {
        log.info('[addOriginGroupDevices] Add devices to origin group with id: %s [%s | %s]', group?.id, group?.name, group?.class)
        if (!group) {
            return false
        }
        ...
        log.info('[addOriginGroupDevices] Serials list is not specified')
...
function putDeviceInfoBySerial(req, res) {
    ...
    return dbapi.loadDeviceBySerial(serial)
        .then((data) => {
            ...
            if (_.has(body, 'displayName')) {
                if (typeof body.displayName !== 'string' || body.displayName.trim().length > 100) {
                    return apiutil.respond(res, 400, 'Device display name must be a string of at most 100 characters')
                }
                updates.push(dbapi.setDeviceDisplayName(serial, body.displayName))
            }
    ...
    function removeDevices(serials) {
        ...
        return Promise.each(serials, function(serial) {
            ...
            .then(function() {
                results = _.without(results, 'unchanged')
                if (!results.length) {
                    return apiutil.respond(res, 409, `Unchanged (${target}): device state does not match filters`)
                }
    ...
    dbapi.initiallySetAdbPort(serial)
        .then(port => {
            if (port) {
                ...
                apiutil.respond(res, 500, 'Adb port update failed: no free port available', {port: null})
            }
        })
        .catch(function(err) {
            apiutil.internalError(res, 'Failed to renew ADB port: ', err.stack)
        })
}
...
async function useDeviceByUser(req, res) {
    try {
        const auth = await accessTokenAuth(req).catch(err => {
        ...
        const user = await dbapi.loadUser(req.body.email)
        const device = await dbapi.loadDevice(user?.groups?.subscribed, req.params.serial)
...
async function bookDevice(req, res) {
    try {
        const auth = await accessTokenAuth(req).catch(err => {
            res.status(err.status).json({message: err.message})
        })
        ...
        if (!duration || typeof duration !== 'number' || duration < 1 || duration > 1440) {
            return apiutil.respond(res, 400, 'Duration must be a number between 1 and 1440 minutes')
        }

        const device = await DeviceModel.loadDeviceBySerial(serial)

        if (!device) {
            return apiutil.respond(res, 404, 'Device not found')
        }

        if (!device.present || !device.ready) {
            return apiutil.respond(res, 400, 'Device is not available')
        }

        if (device.owner && device.owner.email !== req.user.email) {
            return res.status(403).json({
                success: false,
                description: 'Device is currently booked by another user',
                bookedBy: device.owner.name || device.owner.email,
                bookedUntil: device.bookedAt && device.bookedBefore
                    ? new Date(new Date(device.bookedAt).getTime() + device.bookedBefore).toISOString()
                    : null
            })
        }

        const timeoutMs = duration * MILLISECONDS_IN_MINUTE

        await dbapi.enhanceStatusChangedAt(serial, timeoutMs)

        if (!device.owner) {
            await dbapi.setDeviceOwner(serial, {
        ...
        const bookedUntil = new Date(Date.now() + timeoutMs).toISOString()
        ...
        log.error('Failed to book device "%s": ', req.params.serial, err.stack)
        apiutil.respond(res, 500, 'Failed to book device')
    }
}
...
async function releaseBooking(req, res) {
    try {
        ...
        if (device.owner && device.owner.email !== req.user.email) {
            return res.status(403).json({
                success: false,
                description: 'Device is booked by another user'
            })
        }

        await dbapi.enhanceStatusChangedAt(serial, 0)
        await dbapi.setDeviceOwner(serial, null)
        ...
        log.error('Failed to release booking for device "%s": ', req.params.serial, err.stack)
        apiutil.respond(res, 500, 'Failed to release booking')
    }
}
...
export {useDeviceByUser}
export {bookDevice}
export {releaseBooking}
export default {
    ...
    useDeviceByUser: useDeviceByUser,
