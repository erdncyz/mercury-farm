...
import {ConnectStartedMessage, ConnectStoppedMessage, JoinGroupMessage, LeaveGroupMessage, UngroupMessage} from '../../../wire/wire.js'
...
function setTimeoutS(callback, timeout) {
    ...
    return setTimeout(function() {
        try {
            ...
            log.error('Error when executing timeout: ', e?.stack || e)
            return null
        }
    }, timeout)
}
...
function getUser(req, res) {
    ...
    return dbapi.loadGlobalRuntimeSettings()
        .catch((err) => {
            log.error('Failed to load global runtime settings: %s', err?.message || err)
            return {}
        })
        .then((runtimeSettings) => {
            const settings = {..._.omit(req.user?.settings || {}, dbapi.GLOBAL_RUNTIME_SETTINGS_KEYS), ...runtimeSettings}
            res.json({
                success: true,
            ...
            if (lockingSuccessed) {
                ...
                req.options.push.send([
                    ...
                    wireutil.envelope(new wire.GroupMessage(
                        new wire.OwnerMessage(req.user.email, req.user.name, req.user.group),
                        timeout,
