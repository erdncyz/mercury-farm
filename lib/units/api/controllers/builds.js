...
async function getActiveDeviceKeys(builds) {
    ...
    for (const build of builds || []) {
        for (const buildDevice of build.devices || []) {
            ...
            if (device?.owner && (
                device.owner.group === build.id ||
                device.group?.id === build.id
            )) {
...
async function getBuilds(req, res) {
    ...
    try {
        ...
        const activeDeviceKeys = await getActiveDeviceKeys(builds)
        res.json({
            success: true,
            description: 'Builds Information',
            builds: (builds || []).map(build => publishBuild(build, {activeDeviceKeys}))
        })
    }
    catch (err) {
...
function deleteBuild(req, res) {
    ...
    BuildModel.getBuild(id)
        .then(async function(build) {
            ...
            const activeDeviceKeys = await getActiveDeviceKeys([build])
            if (publishBuild(build, {activeDeviceKeys}).state === BUILD_RUNNING_STATE) {
...
function getBuild(req, res) {
    BuildModel.getBuild(req.params.id)
        .then(async function(build) {
            ...
            const isAdmin = req.user.privilege === apiutil.ADMIN
            const activeDeviceKeys = await getActiveDeviceKeys([build])
            const publishedBuild = publishBuild(build, {includeScenarios: true, activeDeviceKeys})
