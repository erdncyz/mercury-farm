...
async function getActiveDeviceKeys(builds) {
    ...
    return new Set((devices || [])
        .filter(device => device.usage === 'automation' && device.owner?.group)
        .map(device => `${device.owner.group}:${device.serial}`))
}
    ...
    BuildModel.getBuild(req.params.id)
        .then(async function(build) {
            ...
            res.json({
                ...
                build: publishBuild(build, {includeScenarios: true, activeDeviceKeys})
            })
        })
        .catch(function(err) {
