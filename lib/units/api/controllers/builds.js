...
import {isDeviceActiveForBuild} from '../../../db/models/build/model.js'
...
function publishBuild(build, options = {}) {
    ...
    const activeDeviceKeys = options.activeDeviceKeys || new Set()
    const devices = (build.devices || []).map(device => ({
        ...device,
        active: activeDeviceKeys.has(`${build.id}:${device.serial}`)
    }))
    const hasActiveDevices = devices.some(device => device.active)
    let state = build.state
    let finishedAt = build.releasedAt || null

    // plannedStop is the initial inactivity lease, not a hard finish time.
    // ADB/WDA traffic may keep the provider lease alive beyond it.
    if (state === BUILD_RUNNING_STATE && plannedStop && plannedStop.getTime() < now && !hasActiveDevices) {
        const lastAliveAt = options.lastAliveAt?.get(build.id)
        state = 'finished'
        finishedAt = finishedAt ||
            (lastAliveAt && lastAliveAt > plannedStop.getTime() ? new Date(lastAliveAt) : build.plannedStop)
    }
...
async function getActivity(builds) {
    const activeDeviceKeys = new Set()
    const lastAliveAt = new Map()
    const serials = [...new Set((builds || []).flatMap(build =>
        (build.devices || []).map(device => device.serial)
    ).filter(Boolean))]

    if (!serials.length) {
        return {activeDeviceKeys, lastAliveAt}
    }

    const devices = await dbapi.loadDevicesBySerials(serials)
    const devicesBySerial = new Map((devices || []).map(device => [device.serial, device]))
    const now = Date.now()

    for (const build of builds || []) {
        for (const buildDevice of build.devices || []) {
            const device = devicesBySerial.get(buildDevice.serial)
            if (isDeviceActiveForBuild(device, build.id, now)) {
                activeDeviceKeys.add(`${build.id}:${buildDevice.serial}`)
            }
            const alive = device?.automationAlive
            if (alive?.group === build.id && alive.at) {
                const aliveAt = new Date(alive.at).getTime()
                lastAliveAt.set(build.id, Math.max(lastAliveAt.get(build.id) || 0, aliveAt))
            }
        }
    }
...
async function getBuilds(req, res) {
    ...
    try {
        const activeGroupIds = await BuildModel.getActiveAutomationGroupIds()
        const filter = req.user.privilege === apiutil.ADMIN ?
            {} :
            {$or: [
                {'owner.email': req.user.email},
                {id: {$in: activeGroupIds || []}}
            ]}
        const builds = await BuildModel.getBuilds(filter, limit)
        const activity = await getActivity(builds)
        res.json({
            success: true,
            description: 'Builds Information',
            builds: (builds || []).map(build => publishBuild(build, activity))
        })
    }
    catch (err) {
...
function deleteBuild(req, res) {
    ...
    BuildModel.getBuild(id)
        .then(async function(build) {
            ...
            const activity = await getActivity([build])
            if (publishBuild(build, activity).state === BUILD_RUNNING_STATE) {
...
function deleteBuilds(req, res) {
    const scope = req.user.privilege === apiutil.ADMIN ?
        {} :
        {'owner.email': req.user.email}
    BuildModel.getActiveAutomationGroupIds()
        .then(function(activeGroupIds) {
            const filter = {$and: [
                scope,
                notRunningFilter(),
                {id: {$nin: activeGroupIds || []}}
            ]}
            return BuildModel.deleteBuilds(filter)
        })
        .then(function(result) {
...
function getBuild(req, res) {
    BuildModel.getBuild(req.params.id)
        .then(async function(build) {
            ...
            const isAdmin = req.user.privilege === apiutil.ADMIN
            const activity = await getActivity([build])
            const publishedBuild = publishBuild(build, {...activity, includeScenarios: true})
            const canWatch = publishedBuild.state === BUILD_RUNNING_STATE
            if (!isAdmin && build.owner?.email !== req.user.email && !canWatch) {
                return apiutil.respond(res, 403, 'Forbidden (build)')
            }
            res.json({
                success: true,
