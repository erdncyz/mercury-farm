...
function addGroupDevices(req, res) {
    ...
    function _addGroupDevices(group, serials) {
        return dbapi.addGroupDevices(group, serials)
            .then(async(autotestsGroup) => {
                const now = new Date()

                if (dbapi.checkDatesOverlaps(
                    {dates: [{start: now, stop: now}]}
                    , [autotestsGroup]
                )) {
                    await dbapi.updateDevicesCurrentGroup(autotestsGroup?.devices, autotestsGroup)
                }

                if (isInternal) {
                    return DeviceModel.loadDevicesBySerials(autotestsGroup?.devices)
            ...
            .catch(function(err) {
                ...
                else if (err === 'dates overlap') {
                    apiutil.respond(res, 403, 'Forbidden (device(s) are busy in the required schedule period)')
                }
    ...
    return dbapi.getUserGroup(req.user.email, id).then(function(group) {
        ...
        if (typeof serials === 'undefined' && typeof amount === 'undefined') {
            return dbapi.getGroups({id: {$in: req.user.groups.subscribed}}).then(function(groups) {
                let exclude = []
                const serials = groups?.flatMap(g => {
                    ...
                    if (!isOriginGroup(g.class)) {
                        const stop = new Date(g.dates[0].stop || '')
                        const start = new Date(g.dates[0].start || '')

                        if ((
                            start < group.dates[0].start &&
                                stop > group.dates[0].start
                        ) || (
                            start > group.dates[0].start &&
                                start < group.dates[0].stop
                        )) {
                            exclude = _.union(exclude, g.devices)
                            return []
                        }
                    }

                    return g.devices.filter(serial =>
                        !group.devices.includes(serial) && !exclude.includes(serial)
                    )
                }) || []
    ...
    dbapi.getUserGroup(req.user.email, id).then(function(group) {
        ...
        return dbapi.removeGroupDevices(group, serialsToRemove).then(function(group) {
            if (group?.class === apiutil.ONCE) {
                dbapi.updateDevicesCurrentGroupFromOrigin(serialsToRemove).then(() => {
                    if (isInternal) {
                        return true
                    }
                    else {
...
function getGroups(req, res) {
    ...
    switch (owner) {
    case true:
        getGenericGroups = dbapi.getOwnerGroups(req.user.email)
        break
    case false:
        getGenericGroups = dbapi.getOnlyUserGroups(req.user.email)
        break
    default:
        if (req.user.privilege === apiutil.ADMIN) {
            getGenericGroups = dbapi.getGroups()
            break
        }
        getGenericGroups = dbapi.getUserGroups(req.user.email)
    }

    getGenericGroups.then(function(groups) {
    ...
    }).catch(err => apiutil.internalError(res, 'Failed to get groups: ', err?.stack || err))
}
...
function deleteGroups(req, res) {
    ...
    function removeGroup(id) {
        function unsetDevices(serials) {
            return Promise.all(serials.map(function(serial) {
                let unsetReq = {
                    isInternal: true,
                    serial: serial,
                    user: req.user,
        ...
        return dbapi.getUserGroup(req.user.email, id).then(function(group) {
            ...
            if (group.class === apiutil.BOOKABLE) {
                return group.devices.reduce((promise, serial) => {
                    ...
                    .then(function() {
                        return unsetDevices(group.devices).then(() => dbapi.deleteUserGroup(id))
                    })
                    .catch(function(err) {
                        if (err !== 'booked') {
                            throw err
        ...
        return dbapi.updateUserGroup(group, data)
            .then(function(group) {
                if (group && typeof group !== 'boolean') {
                    if (isInternal) {
                        DeviceModel.loadDevicesBySerials(group.devices)
        ...
        return checkSchedule(res, group, _class, group.owner.email, repetitions, group.privilege, start, stop)
            .then(function(checkingSuccessed) {
                ...
                if (
                    ...
                    if (isInternal) {
                        return DeviceModel.loadDevicesBySerials(group.devices)
...
function getGroupDevices(req, res) {
    ...
    dbapi.getUserGroup(req.user.email, id).then(function(group) {
        ...
        if (bookable) {
            ...
            DeviceModel.loadDevicesBySerials(group.devices)
                .then(function(devices) {
                    devices = devices.map(device => apiutil.filterDevice(req, device))
