...
function userApiWrapper(fn, req, res) {
    ...
    UserModel.loadUser(email).then(function(user) {
        if (!user) {
...
async function removeUser(email, req, res) {
    ...
    async function removeGroupUser(owner, id) {
        const userGroup = await GroupModel.getUserGroup(owner, id)
        if (!userGroup) {
            return 'not found'
        }
        return owner === email ?
            GroupModel.deleteUserGroup(id) :
            GroupModel.removeGroupUser(id, email)
    }

    function deleteUserInDatabase(channel) {
        return AllModel.removeUserAccessTokens(email).then(function() {
            return UserModel.deleteUser(email).then(function() {
    ...
    return AllModel.lockUser(email).then(function(stats) {
        if (stats.modifiedCount === 0) {
            return apiutil.lightComputeStats(res, stats)
        }
        const user = lock.user = stats.changes[0].new_val
        return GroupModel.getGroupsByUser(user.email).then(function(groups) {
...
function grantAdmin(req, res) {
    ...
    UserModel.grantAdmin(req.params.email).then((user) => {
        if (user) {
            // @ts-ignore
            return apiutil.respond(res, 200, 'Grant admin for user', {user: user})
        }
        else {
            apiutil.respond(res, 404, 'Not Found (user)')
        }
...
function revokeAdmin(req, res) {
    ...
    UserModel.revokeAdmin(req.params.email).then((user) => {
        if (user) {
            // @ts-ignore
            return apiutil.respond(res, 200, 'Revoke admin for user', {user: user})
        }
        else {
            apiutil.respond(res, 404, 'Not Found (user)')
        }
...
function lockStfAdminUser(res) {
    return AllModel.lockUser(apiutil.MERCURY_ADMIN_EMAIL).then(function(stats) {
        if (stats.modifiedCount === 0) {
            return apiutil.lightComputeStats(res, stats)
        }
        return {user: stats.changes[0].new_val}
    })
}
function updateUsersAlertMessage(req, res) {
    ...
    return UserModel.updateUsersAlertMessage(req.body).then(function(/** @type {any} */ stats) {
        if (stats.matchedCount > 0 && stats.modifiedCount === 0) {
            apiutil.respond(res, 200, 'Unchanged (users alert message)', {alertMessage: stats.newUser.settings.alertMessage})
        }
        else {
            apiutil.respond(res, 200, 'Updated (users alert message)', {alertMessage: stats.newUser.settings.alertMessage})
        }
    })
        .catch(function(err) {
...
function getUsersAlertMessage(req, res) {
    const fields = req.query.fields
    return UserModel.loadUser(apiutil.MERCURY_ADMIN_EMAIL).then(async function(user) {
        if (user?.settings?.alertMessage === undefined) {
            const lock = await lockStfAdminUser(res)
            const alertMessage = {
                activation: 'False',
                data: '*** this site is currently under maintenance, please wait ***',
                level: 'Critical'
            }
            return UserModel.updateUsersAlertMessage(alertMessage).then(function(/** @type {any} */ stats) {
                if (stats.modifiedCount === 1) {
                    return stats.newUser.settings.alertMessage
                }
    ...
    UserModel.loadUser(email).then(function(user) {
        if (user) {
            lockutil.lockUser(email, res, lock).then(function(lockingSuccessed) {
                if (lockingSuccessed) {
                    return UserModel.updateUserGroupsQuotas(email, duration, number, repetitions)
                        .then(function(/** @type {any} */ stats) {
                            if (stats.modifiedCount > 0) {
                                return apiutil.respond(res, 200, 'Updated (user quotas)', {
                                    user: apiutil.publishUser(stats.newUser)
                                })
                            }
...
function updateDefaultUserGroupsQuotas(req, res) {
    ...
    lockutil.lockUser(req.user.email, res, lock).then(function(lockingSuccessed) {
        if (lockingSuccessed) {
            return UserModel.updateDefaultUserGroupsQuotas(req.user.email, duration, number, repetitions)
                .then(function(/** @type {any} */ stats) {
                    if (stats.modifiedCount > 0) {
...
function createUser(req, res) {
    ...
    UserModel.createUser(email, name, req.user.ip).then(function(/** @type {any} */ stats) {
...
function deleteUsers(req, res) {
    ...
    (function() {
        if (typeof emails === 'undefined') {
            return UserModel.getEmails().then(function(emails) {
