 ...
 */
const useDevice = ({user, device, channelRouter, push, sub, usage = null, groupId = null, log}) => new Promise(async(resolve, reject) => {
    if (!device) {
        return reject(UseDeviceError.NOT_FOUND)
    }

    const rentTime = (new Date(device.group.lifeTime.stop)).getTime() - Date.now()
    // GroupMessage.timeout is an uint32, so only pass a sane positive value.
    // Anything else lets the device fall back to its own --group-timeout.
    const timeout = Number.isFinite(rentTime) && rentTime > 0 ? Math.floor(rentTime) : 0
