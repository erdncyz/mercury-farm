...
const IOS_NATIVE_SCROLL_MIN_DISTANCE = 0.08
const IOS_NATIVE_SCROLL_AXIS_DOMINANCE = 1.5
const IOS_NATIVE_SCROLL_UP_VELOCITY = 2500
const IOS_NATIVE_SCROLL_DOWN_VELOCITY = 1000
    ...
    .define(async(options, notifier, push, info) => {
        ...
        class WdaClient {
            ...
            swipe(params) {
                ...
                const deltaX = Number(params.toX) - Number(params.fromX)
                const deltaY = Number(params.toY) - Number(params.fromY)
                const absDeltaX = Math.abs(deltaX)
                const absDeltaY = Math.abs(deltaY)
                const durationSeconds = Number(scale.duration)
                const isNativeVerticalScroll = absDeltaY >= IOS_NATIVE_SCROLL_MIN_DISTANCE &&
                    absDeltaY >= absDeltaX * IOS_NATIVE_SCROLL_AXIS_DOMINANCE
                ...
                const requestParams = isNativeVerticalScroll ?
                    {
                        method: 'POST',
                        uri: `${this.baseUrl}/session/${this.sessionId}/wda/swipe`,
                        body: {
                            direction: deltaY < 0 ? 'up' : 'down',
                            velocity: deltaY < 0 ? IOS_NATIVE_SCROLL_UP_VELOCITY : IOS_NATIVE_SCROLL_DOWN_VELOCITY
                        },
                        json: true
                    } :
                    {
                    ...
                    actions: [
                        {
                            ...
                            actions: [
                                ...
                                {type: 'pointerMove', duration: 180, x: startX, y: midY},
                                {type: 'pointerMove', duration: 520, x: startX, y: endY},
