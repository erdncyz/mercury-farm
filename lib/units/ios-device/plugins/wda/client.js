...
const DEFAULT_WDA_REQUEST_TIMEOUT_MS = parseMs(process.env.IOS_WDA_REQUEST_TIMEOUT_MS, 8000)
...
const DEFAULT_IOS_ACTION_TIMEOUT_RECOVERY_THRESHOLD = parseMs(process.env.IOS_ACTION_TIMEOUT_RECOVERY_THRESHOLD, 1)
        ...
        class WdaClient {
            ...
            buildWdaSessionSettings() {
                ...
                const settings = {
                    ...
                    mjpegServerScreenshotQuality: Math.max(1, Math.min(100, DEFAULT_IOS_WDA_MJPEG_QUALITY)),
                    mjpegScalingFactor: Math.max(1, Math.min(100, DEFAULT_IOS_WDA_MJPEG_SCALING))
                }

                if (DEFAULT_IOS_WDA_LEAN_MODE) {
            ...
            getTreeElements() {
                if (DEFAULT_IOS_WDA_TREE_CACHE_MS > 0) {
                    const cacheAge = Date.now() - this.treeElementsCache.fetchedAt
                    if (this.treeElementsCache.value && cacheAge <= DEFAULT_IOS_WDA_TREE_CACHE_MS) {
            ...
            handleRequest(requestOpt) {
                ...
                const timeout = requestOpt?.timeout || (
                    isSessionBootstrapCall
                        ? DEFAULT_WDA_SESSION_TIMEOUT_MS
                        : isActionCall
                            ? DEFAULT_WDA_ACTION_TIMEOUT_MS
                            ...
                            if (!errMes || [
                                ...
                                if (isTimeout) {
                                    ...
                                    if (isActionCall) {
                                        ...
                                        if (this.consecutiveActionTimeouts >= DEFAULT_IOS_ACTION_TIMEOUT_RECOVERY_THRESHOLD && !this.isRecoveringFromActionTimeout) {
