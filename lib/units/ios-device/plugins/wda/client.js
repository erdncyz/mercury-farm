...
export default syrup.serial()
    ...
    .define(async(options, notifier, push, info) => {
        ...
        class WdaClient {
            ...
            upperCase = false
            isSwiping = false
            queuedSwipeParams = null
            swipeInFlightPromise = null
            isRotating = false
            deviceType = null
            orientationPollTimer = null
            orientationPollIntervalMs = 1000
            lastPublishedRotation = null
            getDeviceType() {
                            ...
                            .then((statusResponse) => {
                                ...
                                return this.handleRequest({
                                    ...
                                    .then((sessionResponse) => {
                                        ...
                                        if (!this.sessionId) {
                                            throw new Error('WDA did not return a valid sessionId')
                                        }
                ...
                return this.handleRequest({
                    ...
                    body: {
                        settings: {
                            ...
                            mjpegScalingFactor: 20,
                            mjpegServerFramerate: 15
                        }
                    },
                    json: true,
                }).then(() => {
                    log.info('Applied low-latency MJPEG settings')
                }).catch((err) => {
                    log.warn(`Unable to apply MJPEG settings: ${err?.message || err}`)
                })
            }
            stopSession() {
            ...
            publishRotationFromOrientation() {
                const rotationDegrees = iosutil.orientationToDegrees(this.orientation)
                if (rotationDegrees === null || rotationDegrees === undefined) {
                    return
                }
                if (this.lastPublishedRotation === rotationDegrees) {
                    return
                }
                this.lastPublishedRotation = rotationDegrees
                push.send([
                    wireutil.global,
            ...
            startOrientationWatcher() {
                ...
                this.stopOrientationWatcher()
                this.orientationPollTimer = setInterval(() => {
                    this.getOrientation().catch((err) => {
                        log.warn('Orientation poll failed: %s', err?.message || err)
                    })
                }, this.orientationPollIntervalMs)
            }
            stopOrientationWatcher() {
                if (this.orientationPollTimer) {
            ...
            swipe(params) {
                ...
                const durationMs = Math.max(40, Math.round((scale.duration || 0.08) * 1000))
                const body = {
                    actions: [
                        {
                            type: 'pointer',
                            ...
                            actions: [
                                ...
                                {type: 'pointerMove', duration: durationMs, x: scale.toX, y: scale.toY},
                ...
                if (this.isSwiping) {
                    this.queuedSwipeParams = params
                    return this.swipeInFlightPromise || Promise.resolve()
                }

                this.isSwiping = true
                this.swipeInFlightPromise = this.handleRequest({
                        method: 'POST',
                        uri: `${this.baseUrl}/session/${this.sessionId}/actions`,
                        body,
                        json: true,
                    }).then((response) => {
                        log.info('swipe actions response: ', response)
                    }).catch((err) => {
                        log.warn('swipe actions failed: ', err?.message || err)
                    }).finally(() => {
                        this.isSwiping = false
                        const queuedSwipeParams = this.queuedSwipeParams
                        this.queuedSwipeParams = null
                        if (queuedSwipeParams) {
                            return this.swipe(queuedSwipeParams)
                        }
                        this.swipeInFlightPromise = null
                    })
                return this.swipeInFlightPromise
            }
            touchUp() {
            ...
            tapDeviceTreeElement(message) {
                ...
                return this.handleRequest({
                    ...
                    .then(response => {
                        const element = response?.value?.[0]?.ELEMENT
                        if (!element) {
                            log.warn('tapDeviceTreeElement: no element found for label "%s"', message?.label)
                            return null
                        }
                        return this.handleRequest({
                            method: 'POST',
                    ...
                    .catch(err => {
                        log.error('tapDeviceTreeElement failed: %s', err?.message || err)
                        return null
                    })
            }
            doubleClick() {
            ...
            setVersion(currentSession) {
                ...
                if (!sdkVersion) {
                    log.warn('WDA session response has no sdkVersion; skipping version update')
                    return
                }
            ...
            getOrientation() {
                return this.handleRequest({
                    ...
                    // Some WDA builds intermittently return an empty payload.
                    // Keep last known orientation instead of throwing and breaking iOS stream startup.
                    this.orientation = orientationResponse?.value || this.orientation || 'PORTRAIT'
                    this.publishRotationFromOrientation()
            ...
            rotation(params) {
                ...
                return this.handleRequest({
                    ...
                    log.error('Rotation request failed: %s', err?.message || err)
                    throw err
                }).finally(() => {
                    // Never keep rotation lock on failures, otherwise next rotate requests are ignored.
                ...
                return this.handleRequest({
                    ...
                    .then((batteryInfoResponse) => {
                        if (!batteryInfoResponse?.value) {
                            log.warn('Battery info response is empty, skipping battery update')
                            return
                        }
        ...
        */
        const WDA_MJPEG_MAX_RETRIES = 8
        const WDA_MJPEG_RETRY_DELAY_MS = 1500
        let retry = WDA_MJPEG_MAX_RETRIES
        let reconnectTimer = null
        let reconnecting = false
        let hasSuccessfulMjpegConnection = false
        ...
        function connectToWdaMjpeg(options) {
            ...
            socket.connect(options.mjpegPort, options.wdaHost, () => {
                ...
                hasSuccessfulMjpegConnection = true
                retry = WDA_MJPEG_MAX_RETRIES
                push.send([
                    wireutil.global,
                    wireutil.envelope(new wire.DeviceStatusMessage(options.serial, 3))
                ])
            })
            if (!hasSuccessfulMjpegConnection) {
                // #410: Use status 6 (preparing) only during initial WDA startup.
                push.send([
                    wireutil.global,
        ...
        async function wdaMjpegCloseEventHandler(hadError) {
            ...
            retry -= 1
            notifier.setDeviceTemporaryUnavailable('WdaMjpeg connection is unstable')
            log.warn('WdaMjpeg disconnected for %s, retries left: %d', options.serial, retry)
