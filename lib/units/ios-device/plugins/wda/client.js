...
const DEFAULT_IOS_WDA_MJPEG_SCALING = parseMs(process.env.IOS_WDA_MJPEG_SCALING, 50)
...
const DEFAULT_IOS_WDA_WAIT_FOR_IDLE_TIMEOUT = parseNonNegative(process.env.IOS_WDA_WAIT_FOR_IDLE_TIMEOUT, 0.0)
...
const buildDefaultRuntimeSettings = () => ({
...
export function createPasteboardPayload(text) {
    return {
...
export function isPointerAction(body) {
    const sources = body?.actions
    return Array.isArray(sources) && sources.length > 0 && sources.every(source =>
        source?.type === 'pointer' && Array.isArray(source.actions) && source.actions.length > 0
    )
}

/**
 * Home-button iPhones (SE, 6/7/8 family) have a ~16:9 point size, while Face ID
 * iPhones are taller than 2:1 and iPads are below 1.6:1. On home-button iPhones a
 * swipe up from the bottom edge opens Control Center instead of the app switcher.
 *
 * @param {{width: number, height: number} | null | undefined} size
 * @returns {boolean}
 */
export function hasPhysicalHomeButton(size) {
    ...
    return ratio >= 1.6 && ratio < 2
}

/**
 * Builds a `/wda/pressAndDragWithVelocity` body that drags up from the bottom
 * edge and holds mid-screen, which SpringBoard recognizes as the app switcher
 * gesture. Plain W3C pointer actions cannot express the hold reliably: iOS
 * treats them as a home flick or ignores them.
 *
 * @param {{width: number, height: number}} viewport Size in the current interface orientation
 */
export function buildAppSwitcherGesture({width, height}) {
    const x = Math.round(width / 2)
    return {
        fromX: x,
        fromY: Math.max(1, Math.round(height) - 2),
        toX: x,
...
// A tap released while a swipe is still running on the device is replayed once
// the swipe finishes; past this age the user has moved on, so it is dropped.
export const QUEUED_TAP_MAX_AGE_MS = 3000

/**
 * W3C tap body. A second zero-length pointerMove between down and up makes
 * XCTest take one more accessibility snapshot (~130 ms) without changing the tap.
 *
 * @param {number} x
 * @param {number} y
 */
export function buildTapActions(x, y) {
    return {
        actions: [
            {
                type: 'pointer',
...
/**
 * Decides which queued gesture runs next so taps and swipes replay in the
 * order the user made them.
 *
 * @param {{queuedAt: number} | null} tap
 * @param {{queuedAt: number} | null} swipe
 * @param {number} now
 * @returns {'tap' | 'swipe' | 'stale-tap' | null}
 */
export function nextQueuedGesture(tap, swipe, now = Date.now()) {
    if (tap && now - tap.queuedAt > QUEUED_TAP_MAX_AGE_MS) {
        return 'stale-tap'
    }
    if (tap && (!swipe || tap.queuedAt <= swipe.queuedAt)) {
        return 'tap'
    }
    return swipe ? 'swipe' : null
}

export function getDirectionalSwipe({fromX, fromY, toX, toY}) {
    const deltaX = Number(toX) - Number(fromX)
    const deltaY = Number(toY) - Number(fromY)
    ...
    const secondaryDistance = Math.min(distanceX, distanceY)
    if (dominantDistance < 100 || dominantDistance < secondaryDistance * 1.5) {
        return null
    }
...
/**
 * Creates a WDA session and performs one fast WDA restart when XCTest accepts
 * connections but cannot finish session bootstrap. The restart callback uses
 * the already-built runner, so it does not alter or require persisted signing
 * settings.
 *
 * @param {{
 *   createSession: () => Promise<any>;
 *   restartWda?: () => Promise<any>;
 *   maxAttempts?: number;
 * }} options
 * @returns {Promise<{response: any; sessionId: string}>}
 */
export async function createWdaSessionWithRecovery({
    createSession,
    restartWda,
    maxAttempts = 2
}) {
    ...
    for (let attempt = 1; attempt <= attempts; attempt += 1) {
        try {
            ...
            if (!sessionId) {
                throw new Error('Missing WDA sessionId')
            }
            return {response, sessionId}
        }
        catch (error) {
            lastError = error
            if (attempt >= attempts || typeof restartWda !== 'function') {
                throw error
            }
            await restartWda()
        }
    ...
    throw lastError || new Error('Unable to create WDA session')
}

/**
 * Uses the WDA MJPEG socket as a WDA liveness signal. Brief drops are retried
 * because usbmux tunnels blip (e.g. on USB re-enumeration); `onLost` fires only
 * when the connection cannot be re-established.
 * @param {{
 *   socket: net.Socket;
 *   port: number;
 *   host: string;
 ...
 * }} options
 */
export function watchWdaMjpeg({socket, port, host, log, onLost, attempts = 4, retryDelayMs = 2000, stableMs = 30000}) {
    let retry = attempts
    ...
    const connect = () => {
        ...
        socket.connect(port, host, () => {
            log.info(`Connected to WdaMjpeg ${host}:${port}`)
    ...
    // 'close' always follows 'error'; without a listener a refused reconnect would crash the worker.
    socket.on('error', (err) => log.warn(`WdaMjpeg socket error: ${err?.message || err}`))
    socket.on('close', async(hadError) => {
        clearTimeout(stableTimer)
        if (stopped) {
            return
        }
        log.info(`WdaMjpeg connection was closed${hadError ? ' by error' : ''}`)
        if (!retry) {
            stopped = true
            onLost()
            return
        }
        log.warn(`WdaMjpeg connection lost; reconnecting (${retry} attempts left)`)
...
export default syrup.serial()
    ...
    .define(async(options, notifier, push, info) => {
        ...
        class WdaClient {
            ...
            pendingSwipeQueuedAt = 0
            /** @type {{params: any, isLongPress: boolean, queuedAt: number} | null} */
            pendingTap = null
            touchSwiped = false
            ...
            startSession() {
                ...
                this.sessionStartPromise = (async() => {
                    ...
                    return this.handleRequest({
                        ...
                        .then((sizeOrNull) => {
                            // A missing cached device size does not mean the WDA
                            // session is missing. Creating a second session here
                            // invalidates the reused one while requests are in flight.
                            if (reusedExistingSession) {
                                return sizeOrNull
                            }
                            log.info('starting wda session...')
                            return createWdaSessionWithRecovery({
                                createSession: () => this.handleRequest({
                                    method: 'POST',
                                    uri: `${this.baseUrl}/session`,
                                    body: params,
                                    json: true,
                                }),
                                restartWda: typeof options.restartWda === 'function'
                                    ? async() => {
                                        log.warn('WDA session bootstrap failed; restarting the existing WDA runner')
                                        this.sessionId = null
                                        await options.restartWda()
                                    }
                                    : undefined
                            })
                                .then(({response: sessionResponse, sessionId}) => {
            ...
            buildWdaSessionSettings() {
                const baseScaling = Math.max(1, Math.min(100, this.runtimeSettings.wdaMjpegScaling))
                const effectiveScaling = this.viewportScaling ? Math.min(baseScaling, this.viewportScaling) : baseScaling
                ...
                const settings = {
                    ...
                    mjpegScalingFactor: effectiveScaling,
            ...
            resetRuntimeSettings() {
                this.runtimeSettings = buildDefaultRuntimeSettings()
                this.typeKeyDelay = this.runtimeSettings.typeKeyDelayMs
                this.viewportScaling = null
                log.info('Restored default iOS runtime settings')
                this.applyMjpegSettings().catch((err) => {
                    log.warn('Failed to apply default WDA MJPEG settings: %s', err?.message || err)
                })
            }
            setViewportScaling(scaling) {
                const next = scaling === null ? null : Math.max(20, Math.min(100, Math.round(scaling)))
                if (this.viewportScaling === next) {
                    return
                }
                this.viewportScaling = next
                log.info('Viewport scaling set to %s', next === null ? 'default' : next + '%')
                this.applyMjpegSettings().catch((err) => {
                    log.warn('Failed to apply viewport WDA MJPEG settings: %s', err?.message || err)
                })
            }
            applyMjpegSettings() {
            ...
            homeBtn() {
                if (this.deviceType !== 'Apple TV') {
                    // `pressButton home` reports success but is a no-op on real devices
                    // running iOS 17/26 with the current XCTest; activating SpringBoard
                    // is reliable on every iOS version.
                    return this.handleRequest({
                        method: 'POST',
            ...
            swipe(params) {
                if (this.deviceType !== 'Apple TV') {
                    // The touch-up that ends this gesture must not also fire a tap.
                    this.touchSwiped = true
                }
                return this.performSwipe(params)
            }
            performSwipe(params) {
                const scale = iosutil.swipe(this.orientation, params, this.deviceSize)
                const swipeDirection = getDirectionalSwipe(scale)
                const durationSeconds = Number(scale.duration)
                    ...
                    actions: [
                        {
                            ...
                            actions: [
                                ...
                                {type: 'pointerDown', button: 0},
                                {type: 'pause', duration: swipePressHoldMs},

                                {type: 'pointerMove',
                                    duration: swipeDurationMs,
                                    x: Math.round(scale.toX),
                                    y: Math.round(scale.toY)},
                                {type: 'pointerUp'}
                            ],
                        }
                    ],
                }
                // `/wda/swipe` resolves the active application through several
                // accessibility snapshots. Under a live MJPEG capture this can
                // wedge XCTest's screenshot service. W3C pointer actions send
                // the gesture directly and avoid that snapshot contention.
                const requestParams = swipeDirection
                    ? {
                        method: 'POST',
                        uri: `${this.baseUrl}/session/${this.sessionId}/wda/swipe`,
                        body: {direction: swipeDirection},
                        json: true
                    }
                    : {
                        method: 'POST',
                ...
                let swipeOperation = () => {
                    if (this.isSwiping) {
                        ...
                        if (swipeLockAgeMs > 2500) {
                            ...
                            this.pendingSwipeQueuedAt = Date.now()
                            return
                        }
                    }
                    ...
                    if (!this.isSwiping) {
                        ...
                        this.handleRequest(requestParams).then((response) => {
            ...
            flushPendingGestures() {
                ...
                const swipe = this.pendingSwipeParams ? {queuedAt: this.pendingSwipeQueuedAt} : null
                const next = nextQueuedGesture(tap, swipe)

                if (next === 'stale-tap' && tap) {
                    log.warn('Dropping tap queued %sms ago behind a slow swipe', Date.now() - tap.queuedAt)
                    this.pendingTap = null
                    return this.flushPendingGestures()
                }
                if (next === 'tap' && tap) {
                    this.pendingTap = null
                    // Hold the gesture lock so input arriving meanwhile keeps its order.
                    this.isSwiping = true
                    this.swipeStartedAt = Date.now()
                    return Promise.resolve()
                        .then(() => this.performTap(tap))
                        .catch(err => log.warn('Queued tap failed: %s', err?.message || err))
                        .finally(() => {
                            this.isSwiping = false
                            this.swipeStartedAt = 0
                            this.flushPendingGestures()
                ...
                if (next === 'swipe') {
                    ...
                    return this.performSwipe(pendingSwipeParams)
                }
            }
            touchUp() {
                if (this.touchSwiped || !this.deviceSize) {
                    return
                }
                const tap = {
                    params: this.touchDownParams,
                    isLongPress: !!this.tapStartAt && Date.now() - this.tapStartAt > 1000,
                    queuedAt: Date.now()
                }
                if (this.isSwiping) {
                    // An earlier swipe is still running on the device; replay this
                    // tap after it instead of dropping it.
                    this.pendingTap = tap
                    return
                }
                return this.performTap(tap)
            }
            performTap({params, isLongPress}) {
                if (this.deviceSize) {
                    let {x, y} = params
                    ...
                    if (!isLongPress) {
                        const body = buildTapActions(x, y)
                        if (this.deviceType !== 'Apple TV') {
            ...
            openAppSwitcher() {
                if (this.deviceType === 'Apple TV') {
                    return log.error('App switcher is not supported')
                }
                if (this.isOpeningAppSwitcher) {
                    return Promise.resolve()
                }

                const deviceSize = this.deviceSize || this.size()
                if (!deviceSize) {
                    log.warn('Cannot open the app switcher before the device size is known')
                    return Promise.resolve()
                }

                this.isOpeningAppSwitcher = true
                let request
                if (hasPhysicalHomeButton(deviceSize)) {
                    // The bottom-edge swipe opens Control Center on these devices.
                    request = this.pressButtonSendRequest('home')
                        .then(() => this.pressButtonSendRequest('home'))
                }
                else {
                    const isLandscape = this.orientation === 'LANDSCAPE' ||
                        this.orientation === 'UIA_DEVICE_ORIENTATION_LANDSCAPERIGHT'
                    request = this.handleRequest({
                        method: 'POST',
                        uri: `${this.baseUrl}/session/${this.sessionId}/wda/pressAndDragWithVelocity`,
                        body: buildAppSwitcherGesture({
                ...
                return request.finally(() => {
                    this.isOpeningAppSwitcher = false
                })
            }
            pressPower() {
                return this.handleRequest({
            ...
            setClipBoard(text) {
                return this.handleRequest({
                    method: 'POST',
                    uri: `${this.baseUrl}/session/${this.sessionId}/wda/setPasteboard`,
                    body: createPasteboardPayload(text),
                    json: true
                })
                    .then(response => {
                        if (response == null) {
            ...
            async handleRequest(requestOpt) {
                ...
                const isDirectionalSwipeCall = /\/wda\/swipe$/.test(requestUri)
                const isActionCall = /\/actions$/.test(requestUri)
                    || isDirectionalSwipeCall
                    || /\/wda\/(doubleTap|touchAndHold|pressAndDragWithVelocity)$/.test(requestUri)
                    || /\/element\/.+\/click$/.test(requestUri)
                const shouldPauseScreenCapture = isActionCall &&
                    !isPointerAction(requestOpt?.body) &&
                    !isDirectionalSwipeCall
                ...
                if (shouldPauseScreenCapture) {
                    this.activeActionRequests += 1
                    try {
                        await options.pauseScreenCapture?.()
                    }
                    catch (error) {
                        log.warn('Could not pause iOS screen capture for WDA action: %s', error?.message || error)
                    }
                }
                ...
                return new Promise((resolve, reject) => {
                    requestClient(requestOptions)
                        ...
                        .catch(async(err) => {
                            ...
                            // Session bootstrap must fail loudly so startSession can
                            // restart the wedged XCTest runner and retry once. The old
                            // behavior resolved undefined and surfaced as a misleading
                            // WebSocket authentication error.
                            if (isSessionBootstrapCall) {
                                log.warn('WDA session bootstrap request failed: %s', err?.message || err)
                                reject(err)
                                return
                            }
                    ...
                    if (shouldPauseScreenCapture) {
                        this.activeActionRequests = Math.max(0, this.activeActionRequests - 1)
                        if (this.activeActionRequests === 0) {
                            try {
                                await options.resumeScreenCapture?.()
                            }
                            catch (error) {
                                log.warn('Could not resume iOS screen capture after WDA action: %s', error?.message || error)
                            }
                        }
                    }
                })
            ...
            isGestureActive() {
                return this.isSwiping || this.isRotating || this.activeActionRequests > 0
            }
            recoverScreenCapture(reason = 'screen frame stall') {
                if (this.screenRecoveryPromise) {
                    return this.screenRecoveryPromise
                }
                if (typeof options.restartWda !== 'function') {
                    return Promise.reject(new Error('WDA restart is unavailable'))
                }

                this.screenRecoveryPromise = (async() => {
                    log.warn('Restarting WDA after %s', reason)
                    this.sessionId = null
                    this.sessionStartPromise = null
                    this.sessionRecoveryPromise = null
                    this.isSwiping = false
                    ...
                    this.pendingSwipeParams = null
                    this.pendingTap = null
                    this.activeActionRequests = 0
                    await options.restartWda()
                    await this.startSession()
                    log.info('WDA screen capture recovery completed')
                })().finally(() => {
                    this.screenRecoveryPromise = null
                })
                return this.screenRecoveryPromise
            }
            pressButton(key) {
        ...
        // #410: Use status 6 (preparing) on WDA startup. Only sent once so an
        // MJPEG reconnect does not flip an active session back to preparing.
        push.send([
            wireutil.global,
        ...
        watchWdaMjpeg({
            socket,
            port: options.mjpegPort,
            host: options.wdaHost,
            log,
            onLost: () => {
