    ...
    .define(async(options, notifier, push, info) => {
        ...
        class WdaClient {
            ...
            openAppSwitcher() {
                ...
                const isLandscape = this.orientation === 'LANDSCAPE'
                const width = isLandscape ? this.deviceSize.height : this.deviceSize.width
                const height = isLandscape ? this.deviceSize.width : this.deviceSize.height

                const startX = Math.round(width * 0.5)
                const startY = Math.max(1, Math.round(height * 0.995))
                const midY = Math.max(1, Math.round(height * 0.78))
                const endY = Math.max(1, Math.round(height * 0.36))
                    ...
                    actions: [
                        {
                            ...
                            actions: [
                                ...
                                {type: 'pointerMove', duration: 120, x: startX, y: midY},
