...
import {ApplyIosRuntimeSettingsMessage, BrowserOpenMessage, DashboardOpenMessage, KeyDownMessage, KeyPressMessage, PhysicalIdentifyMessage, RotateMessage, ScreenCaptureMessage, StoreOpenMessage, TapDeviceTreeElement, TouchDownMessage, TouchMoveIosMessage, TouchMoveMessage, TouchUpMessage, TypeMessage} from '../../../../wire/wire.js'
export default syrup.serial()
    .dependency(push)
    .dependency(sub)
    .dependency(wdaClient)
    .dependency(group)
    .define((options, push, sub, wdaClient, group) => {
        const log = logger.createLogger('wda:client')
        const Wda = {}

        group.on('leave', () => {
            wdaClient.resetRuntimeSettings()
            options.setScreenCaptureMode?.('')
        })
        ...
        Wda.connect = () => {
            ...
            sub.on('message', new WireRouter()
                .on(KeyPressMessage, (channel, message) => {
                    // homeBtn() restores portrait coordinates itself after pressing home.
                    Promise.resolve()
                        .then(() => wdaClient.pressButton(message.key))
                        .catch(err => {
                            log.error('Error while pressing button %s: %s', message.key, err?.message || err)
                        })
                })
                ...
                .on(ApplyIosRuntimeSettingsMessage, (channel, message) => {
                    wdaClient.updateRuntimeSettings(message)
                    options.setScreenCaptureMode?.(message.screenCaptureMode || '')
                })
