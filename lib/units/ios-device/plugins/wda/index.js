...
export default syrup.serial()
    ...
    .define((options, push, sub, wdaClient) => {
        ...
        Wda.connect = async () => {
            ...
            const TOUCH_ACTION_TIMEOUT_MS = Number(process.env.IOS_TOUCH_ACTION_TIMEOUT_MS || 7000)
            const TOUCH_RECOVERY_COOLDOWN_MS = Number(process.env.IOS_TOUCH_RECOVERY_COOLDOWN_MS || 10000)
            let isRecoveringTouch = false
            let lastTouchRecoveryAt = 0
            ...
            const canUseCursorDevice = () => !!cursorDevice?.isAvailable?.()

            const recoverTouchChannel = async(reason) => {
                const now = Date.now()
                if (isRecoveringTouch) {
                    return
                }
                if (now - lastTouchRecoveryAt < TOUCH_RECOVERY_COOLDOWN_MS) {
                    return
                }

                isRecoveringTouch = true
                lastTouchRecoveryAt = now
                log.error('iOS touch watchdog triggered: %s', reason)

                try {
                    await wdaClient.stopSession()
                }
                catch (err) {
                    log.warn('Failed to stop WDA session during touch recovery: %s', err?.message || err)
                }

                try {
                    await wdaClient.startSession()
                    log.info('iOS touch watchdog: WDA session restarted successfully')
                }
                catch (err) {
                    log.error('iOS touch watchdog failed to restart WDA session: %s', err?.message || err)
                }
                finally {
                    isRecoveringTouch = false
                }
            }

            const withTimeout = (promiseFactory, timeoutMs, opName) => {
                const timeoutError = new Error(`${opName} timed out after ${timeoutMs}ms`)
                let timeoutId = null
            ...
            let touchActionQueue = Promise.resolve()
            let latestSwipeMessage = null
            let isSwipeDrainScheduled = false
            let shouldSkipNextTouchUp = false

            const runTouchActionWithWatchdog = (opName, action) => {
                return withTimeout(action, TOUCH_ACTION_TIMEOUT_MS, opName)
                    .catch((err) => {
                        return recoverTouchChannel(`${opName}: ${err?.message || err}`)
                    })
            }

            const queueTouchAction = (opName, action) => {
                touchActionQueue = touchActionQueue
                    .catch(() => {})
                    .then(() => runTouchActionWithWatchdog(opName, action))
                return touchActionQueue
            }

            const queueLatestSwipeAction = (message) => {
                latestSwipeMessage = message
                shouldSkipNextTouchUp = true

                if (isSwipeDrainScheduled) {
                    return
                }

                isSwipeDrainScheduled = true

                const drain = () => {
                    const swipeMessage = latestSwipeMessage
                    if (!swipeMessage) {
                        isSwipeDrainScheduled = false
                        return
                    }

                    latestSwipeMessage = null
                    queueTouchAction('swipe', () => wdaClient.swipe(swipeMessage))
                        .finally(() => {
                            setTimeout(drain, 0)
                        })
                }
            ...
            sub.on('message', new WireRouter()
                ...
                .on(TouchDownMessage, (channel, message) => {
                    if (canUseCursorDevice()) {
                        cursorDevice.press()
                    }
                    else {
                        queueTouchAction('tap', () => wdaClient.tap(message))
                    }
                })
                .on(TouchMoveIosMessage, (channel, message) => {
                    if (!canUseCursorDevice()) {
                        queueLatestSwipeAction(message)
                    }
                })
                ...
                .on(TouchUpMessage, (channel, message) => {
                    if (canUseCursorDevice()) {
                        ...
                        if (shouldSkipNextTouchUp) {
                            shouldSkipNextTouchUp = false
                            return
                        }
                        queueTouchAction('touchUp', () => wdaClient.touchUp())
                    }
                })
                .on(TapDeviceTreeElement, (channel, message) => {
                    queueTouchAction('tapDeviceTreeElement', () => wdaClient.tapDeviceTreeElement(message))
                })
                ...
                .on(RotateMessage, (channel, message) => {
                    ...
                    if (!rotation) {
                        log.warn('Unsupported rotation degree for iOS device: %s', message.rotation)
                        return
                    }
