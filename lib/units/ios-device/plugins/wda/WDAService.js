...
export default class WDAService {
    ...
    async prepareWda(simulator, udid) {
        ...
        if (buildProc.exitCode !== 0) {
            throw Error(
                `Could not build WebDriverAgent (xcodebuild exit ${buildProc.exitCode}). ` +
                'If the build log says signing requires a development team, set ' +
                'IOS_WDA_DEVELOPMENT_TEAM (and optionally IOS_WDA_BUNDLE_ID) in ' +
                '.ios-provider.env next to the provider and restart it.'
            )
        }
    }
