...
export function swipe(orientation, params, deviceSize) {
    ...
    case 'LANDSCAPE':
        return {
            fromX: params.fromX * deviceSize.height,
            fromY: params.fromY * deviceSize.width,
            toX: params.toX * deviceSize.height,
            toY: params.toY * deviceSize.width,
            duration: params.duration
        }
    default:
        return {
 ...
 * @returns {string | null} device family name
 */
export function getModelName(identifier) {
    if (recentDeviceNames[identifier]) {
        return recentDeviceNames[identifier]
    }
