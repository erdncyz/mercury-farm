...
import IosScreenCapture from './screen-capture.js'
import {closeSharedMirrorProcesses} from './avcapture-h264-capture.js'
export default syrup.serial()
    ...
    .define(function(options, solo, notifier, WdaClient, push, group) {
        ...
        const webRtcTransport = new H264WebRtcTransport(options, {H264Capture: IosScreenCapture}, {
            platform: 'iOS',
            streamIdPrefix: 'ios',
            captureOptions: {
                url,
                ...
                frameRate: targetFps,
                captureMode: options.screenCaptureMode,
                avCaptureFrameRate: Number(options.screenMirrorFrameRate) || 30,
                avCaptureDeviceName: options.deviceInfo?.name,
        ...
        // Per-user override from Settings > General; '' falls back to the provider default.
        options.setScreenCaptureMode = (mode) => {
            const captureMode = mode || options.screenCaptureMode
            if (webRtcTransport.captureOptions.captureMode === captureMode) {
                return
            }
            log.info('iOS screen capture mode for %s is now %s', options.serial, captureMode)
            webRtcTransport.updateCaptureOptions({captureMode})
        }
        ...
        lifecycle.observe(() => closeSharedMirrorProcesses())
