...
export class Esp32Touch extends EventEmitter {
    ...
    constructor(width, height, portPath) {
        ...
        this.port.write('-')
        this.port.on('open', () => {
            log.info('ESP32 serial port opened: %s', portPath)
        })
        this.port.on('close', () => {
            log.warn('ESP32 serial port closed: %s', portPath)
            this.updateState('disconnected')
            this.stopWorking()
        })
        this.port.on('error', (err) => {
            log.error('ESP32 serial port error: %s', err?.message || err)
    ...
    isAvailable() {
        return this.state === 'paired' && this.port?.isOpen && !this.lastWriteErrorAt
    }
    ...
    bufferCommand(cmd) {
        if (cmd) {
            ...
            this.port.write(cmd, (err) => {
                if (err) {
                    this.lastWriteErrorAt = Date.now()
                    log.error('ESP32 serial write error: %s', err.message)
                    this.updateState('disconnected')
                    this.stopWorking()
                }
            })
