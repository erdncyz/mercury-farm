...
export default (async function(options) {
    ...
    io.on('connection', function(socket) {
        ...
        function sendIosRuntimeSettingsToChannel(channel, runtimeSettings) {
        ...
        function sendAndroidRuntimeSettingsToChannel(channel, runtimeSettings) {
            if (!deviceIsOwned(channel)) {
                return
            }
            ...
            .on(JoinGroupMessage, function(channel, message) {
                const addOwnership = (deviceChannel) => {
                    ...
                    if (message?.owner?.email === user?.email) {
                        ...
                        sendAndroidRuntimeSettingsToChannel(deviceChannel, user?.settings?.androidRuntimeSettings)
                        sendIosRuntimeSettingsToChannel(deviceChannel, user?.settings?.iosRuntimeSettings)
                    }
            ...
            socket.on('disconnect', resolve)
                ...
                .on('user.settings.update', function(data) {
                    if (data.alertMessage === undefined) {
                        ...
                        UserModel.updateUserSettings(apiutil.MERCURY_ADMIN_EMAIL, data)
                    }
                    ...
                    if (data.iosRuntimeSettings && user?.ownedChannels?.size) {
                        user.ownedChannels.forEach((channel) => {
                            sendIosRuntimeSettingsToChannel(channel, data.iosRuntimeSettings)
                        })
                    }
                    if (data.androidRuntimeSettings && user?.ownedChannels?.size) {
                        user.ownedChannels.forEach((channel) => {
                            sendAndroidRuntimeSettingsToChannel(channel, data.androidRuntimeSettings)
                        })
                    }
                })
                            ...
                            .then((device) => {
                                if (device?.channel && device?.owner?.email === user.email) {
                                    ...
                                    if (!user.ownedChannels.has(device.channel)) {
                                        ...
                                        sendAndroidRuntimeSettingsToChannel(device.channel, user?.settings?.androidRuntimeSettings)
                                        sendIosRuntimeSettingsToChannel(device.channel, user?.settings?.iosRuntimeSettings)
                                    }
                                }
                            })
                            .catch((err) => {
