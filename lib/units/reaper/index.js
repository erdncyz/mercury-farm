...
import { DeleteDevice, DeviceAbsentMessage, DeviceHeartbeatMessage, DeviceIntroductionMessage, DevicePresentMessage, GetDeadDevices, GetPresentDevices } from '../../wire/wire.js';
...
export const retryOnTransactionTimeout = async (operation, onRetry, retryDelay = 2000) => {
    for (;;) {
        try {
...
export default (async (options) => {
    ...
    await Promise.all(options.endpoints.sub.map((endpoint) => srv.resolve(endpoint).then(records => srv.attempt(records, (record) => {
    ...
    await Promise.all(options.endpoints.push.map((endpoint) => srv.resolve(endpoint).then(records => srv.attempt(records, (record) => {
        ...
        const deviceCleanerLoop = () => setTimeout(async () => {
            ...
            try {
                ...
                for (const { serial, present } of deadDevices) {
                    ...
                    log.info(// @ts-ignore
                    'Removing a dead device [serial: %s, absence_duration: %.1f %s]', serial, ...(absenceDuration >= 60 // if more 1 hour
    ...
    const init = async () => {
        try {
            ...
            // Load initial state
            const { devices } = await retryOnTransactionTimeout(() => runTransactionDev(wireutil.global, GetPresentDevices, {}, { sub, push, router }), () => log.error('Load initial state error: Timeout when running transaction, retry'));
            const now = Date.now();
