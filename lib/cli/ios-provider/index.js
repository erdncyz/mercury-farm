...
export const builder = function (yargs) {
    return yargs
        ...
        .option('screen-webrtc', {
            describe: 'Enable iOS VideoToolbox H.264/WebRTC with automatic MJPEG fallback.',
            type: 'boolean',
            default: process.env.SCREEN_WEBRTC_ENABLED !== 'false'
        })
        .option('screen-webrtc-bitrate', {
            describe: 'Target iOS H.264 bitrate in bits per second.',
            type: 'number',
            default: Number(process.env.SCREEN_WEBRTC_BITRATE) || 1500000
        })
        .option('screen-webrtc-max-size', {
            describe: 'Maximum iOS H.264 video dimension.',
            type: 'number',
            default: Number(process.env.SCREEN_WEBRTC_MAX_SIZE) || 1280
        })
        .option('screen-capture-mode', {
            describe: 'iOS screen source: mjpeg (WDA screenshots), or experimental auto/avcapture (USB mirroring).',
            type: 'string',
            choices: ['auto', 'avcapture', 'mjpeg'],
            default: process.env.IOS_SCREEN_CAPTURE_MODE || 'mjpeg'
        })
        .option('screen-mirror-frame-rate', {
            describe: 'Target frame rate when the iOS screen comes from USB mirroring.',
            type: 'number',
            default: Number(process.env.IOS_SCREEN_MIRROR_FRAME_RATE) || 30
        })
        .option('screen-webrtc-ice-servers', {
            describe: 'JSON array of STUN/TURN RTCIceServer objects.',
            type: 'string',
            default: process.env.SCREEN_WEBRTC_ICE_SERVERS || '[]'
        })
        .option('screen-webrtc-public-ip', {
            describe: 'Optional public IP advertised as an additional WebRTC host candidate.',
            type: 'string',
            default: process.env.SCREEN_WEBRTC_PUBLIC_IP || ''
        })
        .option('screen-webrtc-port-min', {
            describe: 'Lowest UDP port used by iOS WebRTC peers.',
            type: 'number',
            default: Number(process.env.SCREEN_WEBRTC_PORT_MIN) || 13000
        })
        .option('screen-webrtc-port-max', {
            describe: 'Highest UDP port used by iOS WebRTC peers.',
            type: 'number',
            default: Number(process.env.SCREEN_WEBRTC_PORT_MAX) || 13100
        })
        .option('screen-webrtc-timeout', {
            describe: 'Browser negotiation timeout before MJPEG fallback in milliseconds.',
            type: 'number',
    ...
    return provider({
        ...
        fork: (serial, opts) => {
            const args = [
                ...
                '--screen-webrtc', argv.screenWebrtc,
                '--screen-webrtc-bitrate', argv.screenWebrtcBitrate,
                '--screen-webrtc-max-size', argv.screenWebrtcMaxSize,
                '--screen-capture-mode', argv.screenCaptureMode,
                '--screen-mirror-frame-rate', argv.screenMirrorFrameRate,
                '--screen-webrtc-ice-servers', argv.screenWebrtcIceServers,
                '--screen-webrtc-public-ip', argv.screenWebrtcPublicIp,
                '--screen-webrtc-port-min', argv.screenWebrtcPortMin,
                '--screen-webrtc-port-max', argv.screenWebrtcPortMax,
                '--screen-webrtc-timeout', argv.screenWebrtcTimeout,
