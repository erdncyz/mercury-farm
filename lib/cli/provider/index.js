...
export const builder = function(yargs) {
    return yargs
        ...
        .option('allow-remote', {
            alias: 'R',
            describe: 'Whether to allow remote devices in Mercury. Highly ' +
        ...
        .option('screen-webrtc-bitrate', {
            describe: 'Target Android H.264 bitrate in bits per second.',
            type: 'number',
            default: Number(process.env.SCREEN_WEBRTC_BITRATE) || 1500000
        })
        .option('screen-webrtc-max-size', {
            describe: 'Maximum Android H.264 video dimension.',
            type: 'number',
        ...
        .option('screen-webrtc-port-min', {
            describe: 'Lowest UDP port used by Android WebRTC peers.',
            type: 'number',
            default: Number(process.env.SCREEN_WEBRTC_PORT_MIN) || 13000
        })
        .option('screen-webrtc-port-max', {
            describe: 'Highest UDP port used by Android WebRTC peers.',
            type: 'number',
        ...
        'underscores and prefixing it with `MERCURY_PROVIDER_` (e.g. ' +
        '`MERCURY_PROVIDER_NAME`).')
}
export const handler = function(argv) {
    ...
    validateRange('android worker port', argv.minPort, argv.maxPort)
    validateRange('WebRTC UDP port', argv.screenWebrtcPortMin, argv.screenWebrtcPortMax)
    const workerPorts = range(argv.minPort, argv.maxPort)
    if (workerPorts.length < 2) {
        throw new Error(`Provider needs at least 2 worker ports, got ${workerPorts.length}`)
    }
