...
**Per-user iOS screen mode setting / Kullanıcı bazında iOS ekran modu ayarı** — Settings → General → Device Runtime Settings now has an **iOS Screen Mode** selector with a plain-language explanation of each choice: *Server default* (keeps the provider's `IOS_SCREEN_CAPTURE_MODE`), *Standard* (WDA screenshots, ~10-15 fps, most stable) and *Smooth* (experimental USB video, ~30 fps, USB-connected devices only). The choice is sent to the devices the user takes and applies the next time a device screen is opened; releasing the device restores the provider default.

**Kullanıcı bazında iOS ekran modu ayarı** — Ayarlar → Genel → Cihaz Çalışma Zamanı Ayarları altında artık her seçeneği sade bir dille açıklayan bir **iOS Ekran Modu** seçici var: *Sunucu varsayılanı* (provider'ın `IOS_SCREEN_CAPTURE_MODE` değerini korur), *Standart* (WDA ekran görüntüleri, ~10-15 fps, en kararlı) ve *Akıcı* (deneysel USB video, ~30 fps, yalnızca USB ile bağlı cihazlar). Seçim kullanıcının aldığı cihazlara iletilir ve cihaz ekranı bir sonraki açılışta uygulanır; cihaz bırakıldığında provider varsayılanına dönülür.
