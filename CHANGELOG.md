...
**Yönetici tarafından belirlenen cihaz çalışma zamanı ayarları** — Ayarlar → Genel → Cihaz Çalışma Zamanı Ayarları (Android profili, iOS profili ve yeni **iOS Ekran Modu**) artık yönetici tarafından seçilir ve tüm kullanıcılara uygulanır. Yönetici olmayan kullanıcılar mevcut değerleri salt okunur görür; bu kullanıcılardan gelen değişiklikler backend tarafından reddedilir. Değerler global uyarı mesajıyla birlikte `MERCURY_ADMIN_EMAIL` kullanıcısında saklanır, o anda kullanımda olan tüm cihazlara hemen iletilir, bir kullanıcı cihaz aldığında cihaza gönderilir ve *Ayarları sıfırla* ile değişmez. iOS Ekran Modu seçicisi her seçeneği sade bir dille açıklar: *Sunucu varsayılanı* (provider'ın `IOS_SCREEN_CAPTURE_MODE` değerini korur), *Standart* (WDA ekran görüntüleri, ~10-15 fps, en kararlı) ve *Akıcı* (deneysel USB video, ~30 fps, yalnızca USB ile bağlı cihazlar); cihaz ekranı bir sonraki açılışta uygulanır.
...
**Automation runs no longer show Finished at their timeout while still running / Otomasyon koşumları çalışırken timeout anında artık Finished görünmüyor** — The Builds page and the group scheduler decided whether a run was still active only from the device owner stored in the database. If that owner was cleared mid-run (for example a heartbeat or USB flap briefly marking the device absent), the run flipped to `Finished` at its planned stop (e.g. minute 10 for `timeout=600`) and its group was deleted, even though the test kept running on the device. Devices in an automation group now send an `AutomationAliveMessage` every 30 s; a run stays `Running` while any of its devices is owned by it **or** reported alive within the last 90 s, and closes at the last sign of life once they stop. Runs whose devices were never taken still close at the timeout. Update the providers (Android provider and iOS provider) together with the server; providers that do not send the signal keep the previous behaviour.

**Otomasyon koşumları çalışırken timeout anında artık Finished görünmüyor** — Builds sayfası ve grup zamanlayıcısı koşumun hâlâ aktif olup olmadığına yalnızca veritabanındaki cihaz sahibi (owner) bilgisine bakıyordu. Bu bilgi koşum sırasında silinirse (örneğin kısa bir heartbeat veya USB kopması cihazı bir anlığına absent işaretlerse) test cihazda sürmesine rağmen koşum planlanan bitişte (`timeout=600` için 10. dakika) `Tamamlandı` oluyor ve grubu siliniyordu. Artık otomasyon grubundaki cihazlar her 30 sn'de bir `AutomationAliveMessage` gönderiyor; koşum, cihazlarından biri ona ait olduğu **veya** son 90 sn içinde sinyal gönderdiği sürece `Çalışıyor` kalıyor, sinyaller kesilince son canlılık anında kapanıyor. Hiç cihaz alınmamış koşumlar yine timeout'ta kapanır. Provider'ları (Android provider ve iOS provider) sunucuyla birlikte güncelleyin; sinyal göndermeyen eski provider'larda önceki davranış sürer.
